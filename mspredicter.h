...
class MSPredicter {
 public:
  ...
  MSPredicter(casacore::MeasurementSet &ms, size_t threadCount,
              const Model &model)
      : _ms(ms),
        _startChannel(0),
        _endChannel(0),
  ...
  bool GetNextRow(RowData &data) { return _outputLane.read(data); }
  void FinishRow(RowData &data) { _availableBufferLane.write(data); }
