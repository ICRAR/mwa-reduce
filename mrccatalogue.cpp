...
MRCCatalogue::MRCCatalogue(const std::string& filename)
    : _filename(filename), _file(filename.c_str()) {}
...
bool MRCCatalogue::ReadNext(ModelSource& source) {
  ...
  long double raSec = (entry.raSec[0] - '0') * 10 + (entry.raSec[1] - '0') +
                      (entry.raSec[3] - '0') * 0.1;
  component.SetPosRA(raHrs * (M_PI / 12.0) + raMins * (M_PI / (12.0 * 60.0)) +
                     raSec * (M_PI / (12.0 * 60.0 * 60.0)));
  ...
  component.SetPosDec((decHrs * (M_PI / 180.0) +
                       decMins * (M_PI / (180.0 * 60.0)) +
                       decSec * (M_PI / (180.0 * 60.0 * 60.0))) *
                      decSign);

  long double fluxDensity = digToVal(entry.fluxDensity[0]) * 1000.0 +
