...
void MSPredicter::clearBuffers() {
  for (std::vector<std::complex<double>*>::iterator i = _buffers.begin();
       i != _buffers.end(); ++i)
    delete[] * i;
}
...
void MSPredicter::Start(bool reportSources) {
  ...
  casacore::ROArrayColumn<casacore::Complex> dataColumn(
      _ms, _ms.columnName(casacore::MSMainEnums::DATA));
  ...
  if (!_useModelColumn) {
    ...
    _predicter.reset(new Predicter(
        phaseCentreRA, phaseCentreDec, _bandData.LowestFrequency(),
        _bandData.HighestFrequency(), _bandData.ChannelCount()));
    if (_applyBeam) {
    ...
    if (reportSources) _predicter->ReportSources(_model);
  }
  ...
  // Start all threads
  if (!_useModelColumn) _workThreadGroup.clear();
...
void MSPredicter::ReadThreadFunc() {
  ...
  if (!_useModelColumn) {
    if (_model.SourceCount() == 0) actualThreadCount = 1;
  ...
  casacore::ROScalarColumn<int> ant1Column(
      _ms, _ms.columnName(casacore::MSMainEnums::ANTENNA1));
  casacore::ROScalarColumn<int> ant2Column(
      _ms, _ms.columnName(casacore::MSMainEnums::ANTENNA2));
  casacore::ROArrayColumn<double> uvwColumn(
      _ms, _ms.columnName(casacore::MSMainEnums::UVW));
  casacore::MEpoch::ROScalarColumn timeColumn(
      _ms, _ms.columnName(casacore::MSMainEnums::TIME));
  ...
  if (_useModelColumn) {
    modelColumn.reset(new casacore::ROArrayColumn<casacore::Complex>(
  ...
  for (size_t rowIndex = _startRow; rowIndex != _endRow; ++rowIndex) {
    size_t a1 = ant1Column(rowIndex), a2 = ant2Column(rowIndex);
    ...
    if (a1 != a2) {
      ...
      ++uvwI;
      double w = *uvwI;
      if (_useModelColumn) modelColumn->get(rowIndex, modelData);
      lock.unlock();
      ...
      if (!_useModelColumn && _applyBeam &&
          _beamEvaluator->Time().getValue() != time.getValue()) {
        // Stop all threads, then update beam values, then restart threads.
        _workLane.write_end();
        for (std::thread& t : _workThreadGroup) t.join();
...
void MSPredicter::PredictThreadFunc() {
  ...
  while (_workLane.read(rowData)) {
    ...
    for (size_t ch = _startChannel; ch != _endChannel; ++ch) {
      ...
      _predicter->Predict4(valIter, _model, rowData.u / lambda,
                           rowData.v / lambda, rowData.w / lambda, ch,
