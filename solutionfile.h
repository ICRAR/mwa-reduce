...
/**
 * The solution file is used for storing calibration Jones matrices. The format
 * is as follows: Bytes |  Description
 * -------+---------------
 *  0- 7  |  string intro ; 8-byte null terminated string "MWAOCAL"
 *  8-11  |  int fileType ; always 0, reserved for indicating something other
 * than complex Jones solutions 12-15  |  int structureType ; always 0, reserved
 * for indicating different ordering 16-19  |  int intervalCount ; Number of
 * solution intervals in file 20-23  |  int antennaCount ; Number of antennas
 * that were in the measurement set (but were not necessary all solved for)
 ...
 * always four. 32-39  |  double startTime ; Start time of solutions (AIPS time)
 ...
 * etc.
 *
 * here, ints are always 32 bits unsigned integers, doubles are IEEE double
 * precision 64 bit floating points. If a solution is not available, either
 * because no data were selected during calibration for this interval or because
 * the calibration diverged, a "NaN" will be stored in the doubles belonging to
 * that solution.
 */
class SolutionFile {
 public:
  /** Empty constructor. After constructing, either @ref OpenForReading() should
   * be called or the parameters should be initialized and @ref OpenForWriting()
   * or @ref OpenInMemory() should be called.
   */
  SolutionFile() : _outputStream(), _inputStream(), _readPointer(nullptr) {
    ...
    _header.structureType =
        0;  // ordered real/imag, polarization, channel, antenna, time
  ...
  /** Number of polarizations solved for. This should always be four for now,
   * indicating that full Jones matrices were solved for. */
  ...
  void OpenForWriting(const char *filename) {
    ...
    _outputStream->write(reinterpret_cast<const char *>(&_header),
                         sizeof(_header));
    double timeStart = 0.0, timeEnd = 0.0;
    _outputStream->write(reinterpret_cast<const char *>(&timeStart),
                         sizeof(timeStart));
    _outputStream->write(reinterpret_cast<const char *>(&timeEnd),
                         sizeof(timeEnd));
  }

  /** Open a file for writing and reading. This allows a calibration algorithm
   * to use this class as buffer, without having to store the solutions to disk.
   * After calling this method,
   * @ref WriteSolution() should be used to store all solutions in the memory.
   * After that, the
  ...
  void OpenInMemory() {
    ...
    _data.resize(_header.intervalCount * _header.antennaCount *
                 _header.channelCount * _header.polarizationCount);
    _readPointer = &_data[0];
  }

  /** Open a file for reading from disk. This method will read the header of the
   * file. After calling this method, the settings of this class (@ref
   * IntervalCount(), etc.) are initialized and can be used to determine the
   * number of reads required. After that, the @ref ReadNextSolution() can be
   * used to read the solutions one by one.
   ...
   * This method should be called nIntervals * nAntennas * nChannels * nPols(=4)
   * times. Four reads will give one Jones matrix.
   */
   ...
   * This method can be called in any order, but all values should be written to
   * before closing the file.
   */
  void WriteSolution(const std::complex<double> &val, size_t interval,
                     size_t antenna, size_t channel, size_t polarization) {
    size_t index =
        ((interval * _header.antennaCount + antenna) * _header.channelCount +
         channel) *
            _header.polarizationCount +
        polarization;
