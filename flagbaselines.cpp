...
int main(int argc, char **argv) {
  if (argc < 3 || (argc - 2) % 2 != 0) {
    std::cout << "Usage: flagantennae <ms> <list of baselines>\n"
                 ...
                 "The list of baselines is separated with spaces, and antennae "
                 "are given by index, starting\n"
                 "at zero, e.g.: 'flagbaseline myms.ms 12 15 12 16' will flag "
                 "the baselines 12x15 and 12x16.\n";
  } else {
    ...
    ScalarColumn<int> ant1Column(ms, ms.columnName(MSMainEnums::ANTENNA1));
    ScalarColumn<int> ant2Column(ms, ms.columnName(MSMainEnums::ANTENNA2));
    ...
    std::cout << "Flagging " << baselines.size() << " baselines... "
              << std::flush;
    ...
    for (size_t rowIndex = 0; rowIndex != ms.nrow(); ++rowIndex) {
      ...
      if (a1 > a2) std::swap(a1, a2);
      if (baselines.find(std::pair<size_t, size_t>(a1, a2)) !=
          baselines.end()) {
