...
void test()
{
	...
	assert(Vec(listA, listA+2).size() == 2, "vec.size() == 2");
	assert(Vec(listA, listA+2).size() == 2, "vec.size() == 2");
	...
	assert(Vec(listF, listF+5) == Vec(listF, listF+5), "Vec{1, 2, 3, 4, 5} == Vec{1, 2, 3, 4, 5}");
	assert(!(Vec(listF, listF+5) == Vec(listG, listG+5)), "!(Vec{1, 2, 3, 4, 5} == Vec{1, 2, 3, 4, 5})");
	assert(!(Vec(listF, listF+4) == Vec(listF, listF+5)), "!(Vec{1, 2, 3, 4} == Vec{1, 2, 3, 4, 5})");
	...
	vec.pop_back();
	assert(!(vec != Vec()), "!(vec != Vec())");
	assert(!(Vec(listF, listF+5) != Vec(listF, listF+5)), "V!(Vec{1, 2, 3, 4, 5} != Vec{1, 2, 3, 4, 5})");
	assert(Vec(listF, listF+5) != Vec(listF, listG+5), "Vec{1, 2, 3, 4, 5} != Vec{1, 2, 3, 4, 6}");
	assert(Vec(listF, listF+4) != Vec(listF, listF+5), "Vec{1, 2, 3, 4} != Vec{1, 2, 3, 4, 5}");
	
	// operator<
	assert(!(Vec() < Vec()), "!(Vec() < Vec())");
	assert(!(Vec(1,1) < Vec(1,1)), "!(Vec{1} < Vec{1})");
	assert(!(Vec(1,2) < Vec(1,1)), "!(Vec{2} < Vec{1})");
	assert(Vec() < Vec(1,1), "Vec() < Vec{1}");
	assert(!(Vec(1,1) < Vec()), "!(Vec{1} < Vec()");
	assert(Vec(1,1) < Vec(1,2), "Vec{1} < Vec{2}");
	const int listH[] = {1,2,4};
	assert(Vec(listF,listF+3) < Vec(listH, listH+3), "Vec{1,2,3} < Vec{1,2,4}");

	// operator>
	assert(!(Vec() > Vec()), "!(Vec() > Vec())");
	assert(!(Vec(1,1) > Vec(1,1)), "!(Vec{1} > Vec{1})");
	assert(!(Vec(1,1) > Vec(1,2)), "!(Vec{1} > Vec{2})");
	assert(Vec(listH, listH+3) > Vec(listF,listF+3), "Vec{1,2,4} > Vec{1,2,3}");
...
{
	Vec
		vecA(3, 0, IdAllocater<int>(4)),
		vecB(3, 0, IdAllocater<int>(5)),
		vecC(vecA); // select_on_container_copy_construction will add 10 to id
	vecA[0] = 1; vecA[1] = 2; vecA[2] = 3;
	vecB[0] = 11; vecB[1] = 12; vecB[2] = 13;
