...
void Peeler::Perform() {
  ...
  for (size_t pass = 0; pass != passCount; ++pass) {
    size_t startTimestep = timestepCount * pass / passCount,
    ...
    if (_model.Empty()) {
      ...
      modelColumn.reset(new casacore::ROArrayColumn<complex_t>(
          _ms, _ms.columnName(casacore::MSMainEnums::MODEL_DATA)));
    } else {
      ...
      if (_beamOnSource) {
        if (_model.SourceCount() != 1)
          std::cout << "Warning: To correct for the beam, there should be "
                       "exactly one source in the model";
        ...
        for (size_t ch = 0; ch != channelCount; ++ch) {
          ...
          beamEvaluator->EvaluateApparentToAbsGain(
              component.PosRA(), component.PosDec(), frequency,
              &beamValues[ch * 4]);
    ...
    while (predicter->GetNextRow(rowData)) {
      ...
      if (antenna1 == antenna2) {
        ...
        if (_model.Empty()) modelColumn->get(rowIndex, modelData);
        lock.unlock();
        ...
        if (_model.Empty()) {
          ...
          for (size_t ch = 0; ch != channelCount; ++ch) {
            ...
            if (_beamOnSource) {
              std::complex<double> tempResult[4],
                  doubleData[4] = {dataPtr[chIndex], dataPtr[chIndex + 1],
                                   dataPtr[chIndex + 2], dataPtr[chIndex + 3]};
              Matrix2x2::ATimesB(tempResult, &beamValues[ch * 4], doubleData);
              Matrix2x2::ATimesHermB(doubleData, tempResult,
                                     &beamValues[ch * 4]);
              calMethods[ch]->AddData(doubleData, &weightsPtr[chIndex],
                                      &modelValues[chIndex], antenna1, antenna2,
                                      timeIndex);
            } else {
              calMethods[ch]->AddData(&dataPtr[chIndex], &weightsPtr[chIndex],
                                      &modelValues[chIndex], antenna1, antenna2,
                                      timeIndex);
            }
          }
    ...
    for (size_t ch = 0; ch != channelCount; ++ch) tasks.push(ch);
    ...
    for (size_t i = 0; i != cpuCount; ++i) {
      ...
      threadGroup.emplace_back(&Peeler::calibrateThreadFunction, this,
                               threadData);
    }
    ...
    if (_saveSolutionsFiles) {
      ...
      for (size_t ant = 0; ant != antennaCount; ++ant) {
        ...
        double sumPhaseXX = 0.0, sumPhaseYY = 0.0, sumGainXX = 0.0,
               sumGainYY = 0.0;
        size_t sumCount = 0;
        for (size_t ch = 0; ch != channelCount; ++ch) {
        ...
        antFile << startTimestep << '\t' << (sumGainXX / sumCount) << '\t'
                << sumPhaseXX << '\t' << '\t' << (sumGainYY / sumCount) << '\t'
                << sumPhaseYY << '\n';
        if (ant == 1)
          std::cout << startTimestep << '\t' << (sumGainXX / sumCount) << '\t'
                    << sumPhaseXX << '\t' << '\t' << (sumGainYY / sumCount)
                    << '\t' << sumPhaseYY << '\n';
      }
    }
    ...
    while (predicter->GetNextRow(rowData)) {
      ...
      if (antenna1 != antenna2) {
        ...
        for (size_t ch = 0; ch != channelCount; ++ch) {
          ...
          if (Matrix2x2::IsFinite(solutionsA) &&
              Matrix2x2::IsFinite(solutionsB)) {
            ...
            for (size_t p = 0; p != 4; ++p) flagPtr[ch * 4 + p] = true;
        ...
        if (flagsWereChanged) flagColumn.put(rowIndex, flags);
      }
     ...
     */
    for (size_t ch = 0; ch != channelCount; ++ch) delete calMethods[ch];
  }
}
...
void Peeler::calibrateThreadFunction(Peeler::ThreadData data) {
  ...
  while (!data.tasks->empty()) {
    ...
    if (lastSuccessfulChannel != taskIndex)
      (*(data.calMethods))[taskIndex]->InitSolutions(
          *(*(data.calMethods))[lastSuccessfulChannel]);
    size_t iters = _nIter;
    double limit = _stoppingAccuracy;
    (*(data.calMethods))[taskIndex]->Execute(limit, iters);
    if ((iters >= _nIter || !std::isfinite(limit)) &&
        !(*(data.calMethods))[taskIndex]->OnlySolveRotation()) {
      std::cout << "Recalculating channel " << taskIndex
                << " (accuracy=" << limit << ").\n";
