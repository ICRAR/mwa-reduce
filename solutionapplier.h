...
class SolutionApplier {
 public:
  SolutionApplier()
      : _preset(false),
        _inputColumnName(
            casacore::MeasurementSet::columnName(casacore::MSMainEnums::DATA)),
        _outputColumnName(
            casacore::MeasurementSet::columnName(casacore::MSMainEnums::DATA)) {
  }

  void SetPresets(std::complex<double> xx, std::complex<double> xy,
                  std::complex<double> yx, std::complex<double> yy) {
    _preset = true;
    _presetValues[0] = xx;
    _presetValues[1] = xy;
    _presetValues[2] = yx;
  ...
  void SetToZero(casacore::MeasurementSet& ms) {
    ...
    if (_outputColumnName == _inputColumnName) {
      ...
      copyColumn.reset(
          new casacore::ArrayColumn<casacore::Complex>(ms, _outputColumnName));
      outputColumn = &*copyColumn;
    }
    casacore::IPosition dataShape = dataColumn.shape(0);
    casacore::Array<casacore::Complex> data(dataShape);
    for (casacore::Array<casacore::Complex>::iterator i = data.begin();
         i != data.end(); ++i)
      *i = 0.0;
    for (size_t row = 0; row != ms.nrow(); ++row) outputColumn->put(row, data);
  }
  ...
  void Apply(casacore::MeasurementSet& ms, SolutionFile& solutionFile) {
    ...
    if (_outputColumnName == _inputColumnName) {
      ...
      createColumnIfNotExist(ms, _outputColumnName);
      copyColumn.reset(
          new casacore::ArrayColumn<complex_t>(ms, _outputColumnName));
      outputColumn = &*copyColumn;
    }

    casacore::IPosition dataShape = dataColumn.shape(0);
    unsigned polarizationCount = dataShape[0];
    if (polarizationCount != 4) throw std::runtime_error("Should have 4 pols");
    ...
    if (_preset) {
      ...
      if (solutionFile.AntennaCount() != antennaCount) {
        std::ostringstream s;
        s << "Antenna counts do not match: " << solutionFile.AntennaCount()
          << " in solution file, " << antennaCount << " in MS.";
        throw std::runtime_error(s.str());
      }
      channelBlockCount = solutionFile.ChannelCount();
      if (channelBlockCount > channelCount)
        throw std::runtime_error(
            "Solution file has more channels than the measurement set");
      if (channelCount % channelBlockCount != 0)
        throw std::runtime_error(
            "The number of channels in the measurement set is not divisible by "
            "the number of spectral solutions");
      if (solutionFile.PolarizationCount() != polarizationCount)
        throw std::runtime_error("Polarization counts do not match");
      if (channelCount % solutionFile.ChannelCount() != 0)
        throw std::runtime_error("Channel counts do not match");
      std::cout << " DONE\n";
    }
    std::vector<aocommon::UVector<std::complex<double>>> values(antennaCount);
    ...
    if (channelBlockCount != channelCount) {
      std::cout << "Applying each solution to "
    ...
    for (size_t interval = 0; interval != solutionFile.IntervalCount();
      ...
      size_t intervalTimestepStart =
                 (interval * timestepCount) / solutionFile.IntervalCount(),
             intervalTimestepEnd = ((interval + 1) * timestepCount) /
                                   solutionFile.IntervalCount(),
             intervalRowStart = timestepRows[intervalTimestepStart],
             intervalRowEnd = timestepRows[intervalTimestepEnd];
      std::cout << "- Interval " << (interval + 1) << '/'
                << solutionFile.IntervalCount() << " (" << intervalRowStart
                << '-' << intervalRowEnd << ")\n";
      std::cout << "  Antenna1: " << values[1][(channelBlockCount / 2) * 4]
                << "\n";
      for (size_t rowIndex = intervalRowStart; rowIndex != intervalRowEnd;
           ++rowIndex) {
        // Cross correlation?
        ...
        if (a1 != a2) {
          ...
          for (size_t ch = 0; ch != channelCount; ++ch) {
            ...
            std::complex<double>*solA = &values[a1][cbFileIndex],
            *solB = &values[a2][cbFileIndex];
            std::complex<double> dataVals[4] = {dataPtr[0], dataPtr[1],
                                                dataPtr[2], dataPtr[3]};
 ...
 private:
  void applySolution(std::complex<double>* dataVal,
                     const std::complex<double>* solA,
                     const std::complex<double>* solB) {
    std::complex<double> solATimesData[4];
    aocommon::Matrix2x2::ATimesB(solATimesData, solA, dataVal);
    aocommon::Matrix2x2::ATimesHermB(dataVal, solATimesData, solB);
  }

  void createColumnIfNotExist(casacore::MeasurementSet& ms,
    ...
    if (!ms.tableDesc().isColumn(columnName)) {
      ...
      casacore::ArrayColumnDesc<casacore::Complex> columnDesc(columnName,
                                                              shape);
      try {
