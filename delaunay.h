...
class Delaunay {
 private:
  ...
  class Vertex {
   public:
    ...
    static bool is_smaller(const Vertex* a, const Vertex* b) { return *a < *b; }
    ...
    static double TurnDirection(const Vertex* a, const Vertex* b, double x,
                                double y) {
      return (b->x - a->x) * (y - a->y) - (b->y - a->y) * (x - a->x);
    }
    void Disconnect(TriangleNode* t) {
      for (std::vector<TriangleNode*>::iterator i = triangles.begin();
           i != triangles.end(); ++i) {
        if (*i == t) {
          triangles.erase(i);
          break;
  ...
  class Edge {
   public:
    ...
    bool HasSameVertex(const Edge& other, unsigned v1, unsigned v2) {
      for (v1 = 0; v1 != 2; ++v1) {
        for (v2 = 0; v2 != 2; ++v2)
          if (vertices[v1] == other.vertices[v2]) return true;
      }
      // v1=0; v2=0;
      return false;
    }
    bool Crosses(const Vertex* a, const Vertex* b) {
      if (a == vertices[0] || a == vertices[1] || b == vertices[0] ||
          b == vertices[1])
        return false;
      double x0 = vertices[0]->x, y0 = vertices[0]->y, x1 = vertices[1]->x,
             y1 = vertices[1]->y, s1_x = x1 - x0, s1_y = y1 - y0,
             s2_x = b->x - a->x, s2_y = b->y - a->y;
      double s = (-s1_y * (x0 - a->x) + s1_x * (y0 - a->y)) /
                 (-s2_x * s1_y + s1_x * s2_y),
             t = (s2_x * (y0 - a->y) - s2_y * (x0 - a->x)) /
                 (-s2_x * s1_y + s1_x * s2_y);
  ...
  class TriangleNode {
   public:
    ...
    ~TriangleNode() {
      for (size_t i = 0; i != 3; ++i) {
        delete subNodes[i];
        if (vertices[i] != 0) vertices[i]->Disconnect(this);
      }
      if (parent != 0) {
        for (size_t i = 0; i != 3; ++i) {
          if (parent->subNodes[i] == this) parent->subNodes[i] = 0;
        }
      }
    }
    void Print() {
    ...
    bool IsIn(double x, double y) const {
      for (size_t i = 0; i != 3; ++i) {
        double d =
            Vertex::TurnDirection(vertices[i], vertices[(i + 1) % 3], x, y);
        if (d < 0.0) {  // right turn
          return false;
        }
      }
    ...
    bool IsOutsideConvexHull() const {
      ...
      for (size_t i = 0; i != 3; ++i) {
        if (vertices[i]->outsideConvexHull) ++outsideCount;
      }
    ...
    bool HasConvexEdge() const {
      ...
      for (size_t i = 0; i != 3; ++i) {
        if (vertices[i]->inConvexHull) ++convexCount;
      }
    ...
    bool HasConvexVertex() const {
      return vertices[0]->inConvexHull || vertices[1]->inConvexHull ||
             vertices[2]->inConvexHull;
    }
    bool HasOutsideVertex() const {
      return vertices[0]->outsideConvexHull || vertices[1]->outsideConvexHull ||
    ...
    TriangleNode* GetNode(double x, double y) {
      ...
      if (HasSubNodes()) std::cout << "Didn't find a leaf node in GetNode()!\n";
      return this;
    }
    bool IsClockwise() const {
      return Vertex::IsRightTurn(vertices[0], vertices[1], vertices[2]);
    }
    void TurnAround() { std::swap(vertices[0], vertices[2]); }
    Vertex* vertices[3];
 ...
 public:
  ...
  void Clear() {
    delete _topNode;
    for (std::vector<Vertex*>::iterator i = _vertices.begin();
         i != _vertices.end(); ++i)
      delete *i;
    _vertices.clear();
    for (std::vector<Edge*>::iterator i = _convexHull.begin();
         i != _convexHull.end(); ++i)
      delete *i;
    _convexHull.clear();
    _boundTop = 0;
  ...
  void Triangulate() {
    ...
    for (size_t i = 0; i != _vertices.size(); ++i) {
      ...
      if (_vertices[i]->x < 0.0) _vertices[i]->x += M_PI * 2.0;
    }
    ...
    if (deltaX > M_PI * 0.5) {
      ...
      std::cout << "Detected phase wrapping: dist=" << deltaX
                << ", vertex=" << phaseWrappingVertex
                << ", coord=" << kvisCoord(*_vertices[phaseWrappingVertex])
                << '\n';
#endif
    ...
    for (std::vector<Vertex*>::const_reverse_iterator i =
             _vertices.rbegin() + 1;
         i != _vertices.rend(); ++i) {
      while ((*i)->IsLeftTurn(convexVertices)) convexVertices.pop_back();
    ...
    for (std::vector<Vertex*>::const_iterator i = convexVertices.begin() + 1;
         i != convexVertices.end(); ++i) {
      (*i)->inConvexHull = true;
    ...
    for (size_t i = 1; i != _vertices.size(); ++i) {
      ...
      if (v->y < bottomVertex->y) bottomVertex = v;
    }
    Vertex* rightOfTop = 0;
    for (size_t i = topVertexIndex + 1; i < _vertices.size(); ++i) {
      if ((rightOfTop == 0 || rightOfTop->y < _vertices[i]->y) &&
          _vertices[i]->inConvexHull)
        rightOfTop = _vertices[i];
    }
    if (rightOfTop == 0) rightOfTop = topVertex;
    double margin = 1.0, eTopX = topVertex->x, eTopY = topVertex->y,
           eTopXR = rightOfTop->x, eTopYR = rightOfTop->y,
           eBottomY = bottomVertex->y - margin,
           eRightX = eTopX +
                     (eBottomY - eTopY) * (eTopXR - eTopX) / (eTopYR - eTopY) +
                     margin;
    ...
    for (size_t i = 0; i != _vertices.size(); ++i) {
      if (_vertices[i]->inConvexHull) {
        double coef =
            (_vertices[i]->x - eRightX) / (_vertices[i]->y - eBottomY);
...
#ifdef CHECK_DELAUNAY
    std::cout << "Left and right: " << nearestBottom->x << ','
              << nearestBottom->y << "  " << rightOfTop->x << ','
              << rightOfTop->y << '\n';
#endif
    _boundTop = topVertex;
    ...
    _boundRight.x = eRightX;
    _boundRight.y = eBottomY;
    _boundRight.outsideConvexHull = true;
#ifdef CHECK_DELAUNAY
    std::cout << "Bounding triangle: " << _boundLeft.x << ',' << _boundLeft.y
              << "  " << _boundRight.x << ',' << _boundRight.y << "  "
              << _boundTop->x << ',' << _boundTop->y << '\n';
#endif
    _topNode = makeTriangle(0, &_boundLeft, &_boundRight, _boundTop);
#ifdef CHECK_DELAUNAY
    if (_topNode->IsClockwise()) std::cout << "Top node is CW!\n";
#endif
    for (std::vector<Vertex*>::iterator v = _vertices.begin();
         v != _vertices.end(); ++v) {
      if (*v != _boundTop) {
        TriangleNode* node = _topNode->GetNode((*v)->x, (*v)->y);
        for (size_t i = 0; i != 3; ++i) {
          node->subNodes[i] = makeTriangle(node, node->vertices[i],
                                           node->vertices[(i + 1) % 3], *v);
#ifdef CHECK_DELAUNAY
          if (node->subNodes[i]->IsClockwise()) std::cout << "Made CW node!\n";
#endif
        }
      }
    }
    ...
    for (std::vector<Edge>::iterator e = edges.begin(); e != edges.end(); ++e) {
      if (!isLocallyDelaunay(*e)) nonDelaunay.push(new Edge(*e));
    }
#ifdef CHECK_DELAUNAY
  ...
  void SaveConvexHullAsKvis(const std::string& filename) {
    ...
    file << "COLOR RED\n";
    for (std::vector<Edge*>::const_iterator e = _convexHull.begin();
         e != _convexHull.end(); ++e) {
  ...
  void SaveTriangulationAsKvis(const std::string& filename) {
    ...
    while (!nodeStack.empty()) {
      ...
      if (node->HasSubNodes()) {
        for (size_t i = 0; i != 3; ++i) {
          if (node->subNodes[i] != 0) nodeStack.push_back(node->subNodes[i]);
        }
      } else {
        for (size_t i = 0; i != 3; ++i) {
          file << "LINE " << kvisCoord(*node->vertices[i]) << ' '
               << kvisCoord(*node->vertices[(i + 1) % 3]) << '\n';
          file2 << node->vertices[i]->x << '\t' << node->vertices[i]->y << "\n";
        }
        file2 << node->vertices[0]->x << '\t' << node->vertices[0]->y << "\n";
        file2 << '\n';
      }
    }
  }
 ...
 private:
  ...
  static TriangleNode* makeTriangle(TriangleNode* parent, Vertex* a, Vertex* b,
                                    Vertex* c) {
  ...
  bool isLocallyDelaunay(const Edge& edge) {
    ...
    for (size_t i = 0; i != 3; ++i) {
      ...
      if (triangles[1]->vertices[i] != a && triangles[1]->vertices[i] != b)
        d = triangles[1]->vertices[i];
    }
    if ((isBound(a) && isBound(b)) || d == 0) return true;
    ...
    return isLocallyDelaunay(triangles[0]->vertices[2],
                             triangles[0]->vertices[1],
                             triangles[0]->vertices[0], d);
  }
  ...
  bool flip(const Edge& edge, Vertex** vertices) {
    ...
    Vertex *c = 0, *d = 0;
    for (size_t i = 0; i != 3; ++i) {
      if (triangles[0]->vertices[i] != a && triangles[0]->vertices[i] != b)
        c = triangles[0]->vertices[i];
      if (triangles[1]->vertices[i] != a && triangles[1]->vertices[i] != b)
        d = triangles[1]->vertices[i];
    }
  ...
  static bool isLocallyDelaunay(const Vertex* a, const Vertex* b,
                                const Vertex* c, const Vertex* d) {
    return circumDeterminant(a, b, c, d) >= 0.0;
  }

  static double circumDeterminant(const Vertex* a, const Vertex* b,
                                  const Vertex* c, const Vertex* d) {
    double ma = a->x - d->x, mb = a->y - d->y,
           mc = a->x * a->x - d->x * d->x + a->y * a->y - d->y * d->y,
           md = b->x - d->x, me = b->y - d->y,
           mf = b->x * b->x - d->x * d->x + b->y * b->y - d->y * d->y,
           mg = c->x - d->x, mh = c->y - d->y,
           mi = c->x * c->x - d->x * d->x + c->y * c->y - d->y * d->y;
    return (ma * me * mi + mb * mf * mg + mc * md * mh) -
  ...
  void getEdges(std::vector<Edge>& edges) {
    for (std::vector<TriangleNode*>::const_iterator i = _triangles.begin();
         i != _triangles.end(); ++i) {
      TriangleNode* t = *i;
