...
struct TimingRecord {
  ...
  std::string ToLine(const std::string& description, const double val) {
    ...
    newLine << description << '\t' << val << '\t' << average << '\t' << stddev
            << '\t' << n << '\t' << sum << '\t' << sumSq;
    return newLine.str();
  }
};

bool parseLine(const std::string& line, std::string& description, double& value,
               TimingRecord& record) {
  std::string tokens[7];
  size_t curPos = 0;
  for (size_t i = 0; i != 6; ++i) {
    size_t delimitor = line.find('\t', curPos);
    if (delimitor == std::string::npos) return false;
...
int main(int argc, char* argv[]) {
  if (argc < 5) {
    std::cout << "Syntax: watch <file> <task-description> <task-value> <cmd> "
                 "[cmd-params ..]\n";
    return -1;
  }
  ...
  std::cout << "Task \"" << taskDescription << "\" finished in "
            << watch.ToString() << ".\n";
    ...
    if (!line.empty()) {
      ...
      if (parseLine(line, curDesc, curVal, record)) {
        if (!taskFound && curDesc == taskDescription) {
          ...
          record.Recalculate();
          data.insert(
              std::make_pair(curVal, record.ToLine(taskDescription, curVal)));
        } else {
  ...
  if (!taskFound) {
    ...
    data.insert(
        std::make_pair(taskValue, record.ToLine(taskDescription, taskValue)));
  }
  file.close();

  std::ofstream ofile(filename);
  for (std::map<double, std::string>::const_iterator i = data.begin();
       i != data.end(); ++i)
    ofile << i->second << '\n';
}
