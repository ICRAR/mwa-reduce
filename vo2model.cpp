...
double getAsDouble(const std::string& data) { return atof(data.c_str()); }
...
void readValue(const std::string& columnName, const std::string& value) {
  if (columnName == "Name")
    ...
    spectInd = getAsDouble(value);
           ...
           columnName == parserData.secondFluxCol)
    secondFlux = getAsDouble(value);
  else if (parserData.useThirdFrequency &&
           columnName == parserData.thirdFluxCol)
...
void startRow() {
  ...
  if (!parserData.useSIFormat) source.front().SetSED(MeasuredSED());
...
void endRow() {
  if (parserData.useSIFormat) {
    ...
    if (!std::isfinite(flux)) {
      if (removed < 100)
        std::cout << "Warning: " << source.Name()
                  << " has no flux: removing.\n";
      ++removed;
    } else {
      PowerLawSED sed;
      ...
      aocommon::UVector<double> slopes{spectInd};
...
int main(int argc, char* argv[]) {
  if (argc < 5) {
    std::cerr
        << "Syntax: vo2model [-si <val>] [-sicol <SI col>] [-flux2 <colname> "
           "<MHz>] <flux col> <frequency in MHz> <in.vot> <out.txt>\n";
    return -1;
  }
  ...
  std::string fluxDensityColumn(argv[argi]), inFile(argv[argi + 2]),
      outFile(argv[argi + 3]);
  double frequency = atof(argv[argi + 1]) * 1e6;
