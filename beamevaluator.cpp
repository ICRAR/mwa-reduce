...
BeamEvaluator::BeamEvaluator(casacore::MeasurementSet& ms, bool reportDelays,
  ...
  double delays[16];
  if (reportDelays) std::cout << "Delays: [";
  for (int i = 0; i != 16; ++i) {
  ...
  if (reportDelays) std::cout << "]\n";
  _tileBeam.reset(new TileBeam(delays, false, mwaPath));
}

void BeamEvaluator::EvaluateAbsToApparentGain(double ra, double dec,
                                              double frequency,
...
#ifdef CUDA_SUPPORT
void BeamEvaluator::EvaluateAbsToApparentGain(
    const std::vector<PrecalcPosInfo>& posInfo, std::complex<double>* gains,
    size_t startChannel, size_t endChannel, size_t channelCount,
    size_t startFrequency, size_t endFrequency) {
  ...
  _tileBeam->ArrayResponse(ze, az, n_components, gains, startChannel,
                           endChannel, channelCount, startFrequency,
                           endFrequency);
  delete[] az;
  delete[] ze;
}
