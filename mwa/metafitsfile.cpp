...
MetaFitsFile::MetaFitsFile(const char* filename) : _filename(filename)
{
	int status = 0;
	fits_open_file(&_fptr, filename, READONLY, &status);
	checkStatus(status, _filename, "Open metafits file");
...
void MetaFitsFile::ReadTiles(std::vector<MWAInput>& inputs, std::vector<MWAAntenna>& antennae)
{
	...
	for(long int i=0; i!=nrow; ++i)
	{
		...
		if(pol == 'X')
		{
			...
			ant.position[0] = 0.0; ant.position[1] = 0.0; ant.position[2] = 0.0;
