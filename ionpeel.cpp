...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    std::cout << "syntax: ionpeel [options] <ms> <model> <solutionfile>\n"
                 ...
                 " -minuv <dist in m>\n"
                 "\tDon't use baselines < dist in the solutions (they will be "
                 "peeled though).\n"
                 " -j <cpucount>\n"
  ...
  size_t solutionInterval = 1,
         threadCount = (size_t)sysconf(_SC_NPROCESSORS_ONLN);
  ...
  if (clusterFluxLimit != 0.0 && outModelFilename.empty()) {
    throw std::runtime_error(
        "You have not specified an output model filename (with -savemodel) but "
        "have specified a cluster flux limit (-climit): you will not be able "
        "to apply the solutions without the changed model!");
  }
  ...
  if (clusterFluxLimit != 0.0) peeler.SetClusterFluxLimit(clusterFluxLimit);
  if (distLimit != 0.0) peeler.SetDistanceLimit(distLimit);
