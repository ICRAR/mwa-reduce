...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cout
        << "Usage: subtrmodel [-usemodelcol] [-mwapath <PATH>] [-datacolumn "
           "<COLUMN>] [-applybeam] [-r / -s] [-n <σ>] <model> <ms>\n"
           "Subtracts the model from the visibilities. This 'peels' the\n"
           "sources out. Only affects cross-correlations. -r to revert or -s "
           "to set.\n"
           "mwapath is used to specify the path the FEE beam model of MWA.\n";
  } else {
    ...
    if (useModelCol) {
      ...
      casacore::ROArrayColumn<casacore::Complex> modelColumn(
          ms, casacore::MeasurementSet::columnName(
                  casacore::MeasurementSet::MODEL_DATA));
      casacore::Array<casacore::Complex> dataArr(dataColumn.shape(0)),
          modelArr(modelColumn.shape(0));
      for (size_t i = 0; i != ms.nrow(); ++i) {
        ...
        for (casacore::Array<casacore::Complex>::const_contiter m =
                 modelArr.cbegin();
             m != modelArr.cend(); ++m, ++d) {
          *d -= *m;
        }
