...
void Predicter::initialize(ModelComponent& component) {
  ...
  NumType l, m;
  aocommon::ImageCoordinates::RaDecToLM<NumType>(
      component.PosRA(), component.PosDec(), _ra0, _dec0, l, m);
  parameters->l = l;
  parameters->m = m;
  ...
  if (component.Type() == ModelComponent::GaussianSource) {
    ...
    // sigmamaj/min are multiplications and include pi^2 factor, because the
    // sigma of the Fourier transform of a Gaus is 1/sigma of the normal Gaus
    // and has a sqrt(2 pi^2) factor.
  ...
  for (size_t ch = 0; ch != _channelCount; ++ch) {
    ...
    for (size_t p = 0; p != 4; ++p) {
      stokesValues[p] = component.SED().FluxAtChannel(
          ch, _channelCount, _startFrequency, _endFrequency,
          aocommon::Polarization::IndexToStokes(p));
    }
    aocommon::Polarization::StokesToLinear(stokesValues,
                                           &parameters->brightness[ch * 4]);
  }
  ...
  for (size_t ch = 0; ch != _channelCount; ++ch) {
    CNumType temp[4];
    aocommon::Matrix2x2::ATimesB(temp, &parameters->beamValues[ch * 4],
                                 &parameters->brightness[ch * 4]);
    aocommon::Matrix2x2::PlusATimesHermB(_totalFlux, temp,
                                         &parameters->beamValues[ch * 4]);
  }
...
void Predicter::updateBeam(ModelComponent& component, size_t startChannel,
                           size_t endChannel) {
  SourceParameters* parameters =
      reinterpret_cast<SourceParameters*>(component.UserData());
  BeamEvaluator::PrecalcPosInfo posInfo;
  if (_beamEvaluator != nullptr) {
    _beamEvaluator->PrecalculatePositionInfo(posInfo, component.PosRA(),
                                             component.PosDec());
  }
  ...
  _threads.For(startChannel, endChannel, [&](size_t ch, size_t /*thread*/) {
    if (_beamEvaluator != nullptr) {
      double chCentreFreq =
          (_channelCount > 1)
              ? (_startFrequency + (long double)ch *
                                       (_endFrequency - _startFrequency) /
                                       (long double)(_channelCount - 1))
              : _startFrequency;
      _beamEvaluator->EvaluateAbsToApparentGain(
          posInfo, chCentreFreq, &parameters->beamValues[ch * 4]);
      CNumType temp[4];
      aocommon::Matrix2x2::ATimesB(temp, &parameters->beamValues[ch * 4],
                                   &parameters->brightness[ch * 4]);
      aocommon::Matrix2x2::ATimesHermB(&parameters->appBrightness[ch * 4], temp,
                                       &parameters->beamValues[ch * 4]);
    } else {
      parameters->beamValues[ch * 4 + 0] = 1.0;
      parameters->beamValues[ch * 4 + 1] = 0.0;
      parameters->beamValues[ch * 4 + 2] = 0.0;
      parameters->beamValues[ch * 4 + 3] = 1.0;
      parameters->appBrightness[ch * 4 + 0] = 1.0;
      parameters->appBrightness[ch * 4 + 1] = 0.0;
      parameters->appBrightness[ch * 4 + 2] = 0.0;
      parameters->appBrightness[ch * 4 + 3] = 1.0;
    }
  });
}

void Predicter::UpdateBeam(Model& model, size_t startChannel,
                           size_t endChannel) {
#ifdef BEAM_DEBUG
...
#ifdef BEAM_DEBUG
      SourceParameters* parameters =
          reinterpret_cast<SourceParameters*>(i->UserData());
      // #ifdef BEAM_DEBUG_BIN dumpbeam.write((char*) parameters->beamValues,
      // (endChannel - startChannel) * 4 * sizeof(CNumType)); #endif
      for (size_t ch = 0; ch < (endChannel - startChannel); ch++) {
        dumpbeam << "beamval: [" << parameters->beamValues[ch * 4] << ", "
                 << parameters->beamValues[ch * 4 + 1] << ", "
                 << parameters->beamValues[ch * 4 + 2] << ", "
                 << parameters->beamValues[ch * 4 + 3] << "]" << std::endl;
      }
#endif
    }
  }
...
void Predicter::initialize_one(ModelComponent &component) {
  ...
  component.SetUserData(parameters);
  NumType l, m;
  ImageCoordinates::RaDecToLM<NumType>(component.PosRA(), component.PosDec(),
                                       _ra0, _dec0, l, m);
  parameters->l = l;
  parameters->m = m;
  parameters->brightness = new CNumType[_channelCount * 4];
  ...
  for (size_t ch = 0; ch != _channelCount; ++ch) {
    ...
    for (size_t p = 0; p != 4; ++p) {
      stokesValues[p] = component.SED().FluxAtChannel(
          ch, _channelCount, _startFrequency, _endFrequency,
          Polarization::IndexToStokes(p));
    }
    Polarization::StokesToLinear(stokesValues, &parameters->brightness[ch * 4]);
  }
}

void Predicter::initialize_three(
    std::vector<ModelComponent *> &all_components) {
  for (ModelComponent *c : all_components) {
    SourceParameters *parameters =
        reinterpret_cast<SourceParameters *>(c->UserData());
    for (size_t ch = 0; ch != _channelCount; ++ch) {
      CNumType temp[4];
      Matrix2x2::ATimesB(temp, &parameters->beamValues[ch * 4],
                         &parameters->brightness[ch * 4]);
      Matrix2x2::PlusATimesHermB(_totalFlux, temp,
                                 &parameters->beamValues[ch * 4]);
    }
    parameters->lmsqrt = sqrt(1.0 - parameters->l * parameters->l -
...
void Predicter::updateBeam(std::vector<ModelComponent *> &all_components,
  ...
  if (_beamEvaluator != nullptr) {
    for (size_t i = 0; i < all_components.size(); i++) {
      _beamEvaluator->PrecalculatePositionInfo(
          posInfo[i], all_components[i]->PosRA(), all_components[i]->PosDec());
    }
  }
  CNumType *beamValues =
      new CNumType[_channelCount * 4 * all_components.size()];
  if (_beamEvaluator != nullptr) {
    _beamEvaluator->EvaluateAbsToApparentGain(posInfo, beamValues, startChannel,
                                              endChannel, _channelCount,
                                              _startFrequency, _endFrequency);
  }
  // std::cerr << "Inside update beams, after evaluate\n"<< std::endl;
  for (size_t i = 0; i < all_components.size(); i++) {
    SourceParameters *parameters =
        reinterpret_cast<SourceParameters *>(all_components[i]->UserData());
    for (size_t ch = startChannel; ch < endChannel; ch++) {
      if (_beamEvaluator != nullptr) {
        // memcpy(&parameters->beamValues[ch*4],
        // &beamValues[ch*(all_components.size()*4) + i*4],
        // sizeof(std::complex<double>) * 4);
        memcpy(&parameters->beamValues[ch * 4],
               &beamValues[i * ((endChannel - startChannel) * 4) + ch * 4],
               sizeof(std::complex<double>) * 4);
        // std::cout << "{CDP BV} component " << i << ", channel " << ch << ",
        // beamval: [" << parameters->beamValues[ch*4] << ", " <<
        // parameters->beamValues[ch*4 + 1] << ", " <<
        // parameters->beamValues[ch*4 + 2] << ", " <<
        // parameters->beamValues[ch*4 + 3] << "]" << std::endl;
        CNumType temp[4];
        Matrix2x2::ATimesB(temp, &parameters->beamValues[ch * 4],
                           &parameters->brightness[ch * 4]);
        Matrix2x2::ATimesHermB(&parameters->appBrightness[ch * 4], temp,
                               &parameters->beamValues[ch * 4]);
      } else {
        parameters->beamValues[ch * 4 + 0] = 1.0;
        parameters->beamValues[ch * 4 + 1] = 0.0;
...
void Predicter::ReportSources(Model& model) {
  std::cout << "Model predicter initialized with " << model.SourceCount()
            << " sources of apparent brightness [";
  ...
  std::cout << model.TotalFlux(_startFrequency, _endFrequency,
                               aocommon::Polarization::StokesI);
  std::cout << ','
            << model.TotalFlux(_startFrequency, _endFrequency,
  ...
  std::cout << "], abs at low freq: "
            << model.TotalFlux(_startFrequency, aocommon::Polarization::StokesI)
            << ", abs at high freq: "
            << model.TotalFlux(_endFrequency, aocommon::Polarization::StokesI)
            << ")\n";
}
...
void Predicter::predict4(CNumType* dest, const ModelComponent& component,
  ...
  if (component.Type() == ModelComponent::GaussianSource) {
    ...
    NumType uTransf =
                u * parameters.gausTransf[0] + v * parameters.gausTransf[1],
            vTransf =
                u * parameters.gausTransf[2] + v * parameters.gausTransf[3];
    // std::cout << uTransf << ',' << vTransf << '\n';
    NumType g = exp(-uTransf * uTransf - vTransf * vTransf);
    for (size_t p = 0; p != 4; ++p) dest[p] *= g;
  }
}

void Predicter::Predict4(Predicter::CNumType* dest, const ModelSource& source,
                         Predicter::NumType u, Predicter::NumType v,
                         Predicter::NumType w, size_t channelIndex, size_t a1,
                         size_t a2) {
  for (size_t p = 0; p != 4; ++p) dest[p] = 0.0;
  for (ModelSource::const_iterator i = source.begin(); i != source.end(); ++i) {
    CNumType temp[4];
    predict4(dest, *i, u, v, w, channelIndex, a1, a2);
    for (size_t p = 0; p != 4; ++p) dest[p] += temp[p];
  }
}

void Predicter::Predict4(CNumType* dest, const Model& model, NumType u,
