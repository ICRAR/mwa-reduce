...
class Cluster {
 public:
  ...
  void RemoveSource(const ModelSource* source) {
    for (std::vector<const ModelSource*>::iterator i = _sources.begin();
         i != _sources.end(); ++i) {
      if (*i == source) {
        _sources.erase(i);
        return;
  ...
  void RecalculateMean() {
    if (!_sources.empty()) {
      ...
      for (std::vector<const ModelSource*>::const_iterator i = _sources.begin();
           i != _sources.end(); ++i)
        ras.push_back((*i)->Peak().PosRA());
      _meanRA = aocommon::ImageCoordinates::MeanRA(ras);
      _meanDec = 0.0;
      for (std::vector<const ModelSource*>::const_iterator i = _sources.begin();
           i != _sources.end(); ++i) {
  ...
  long double Distance(const ModelSource& source) const {
    ...
    /*long double dRA = source.Peak().PosRA() - _meanRA, dDec =
    source.Peak().PosDec() - _meanDec; while(dRA > 2.0*M_PI) dRA -= 2.0*M_PI;
  ...
  long double AngularDistance(const ModelSource& source) const {
    return aocommon::ImageCoordinates::AngularDistance<long double>(
        source.Peak().PosRA(), source.Peak().PosDec(), _meanRA, _meanDec);
  }
  ...
  long double TotalFlux() const {
    double sum = 0.0;
    for (std::vector<const ModelSource*>::const_iterator s = _sources.begin();
         s != _sources.end(); ++s) {
      double flux = (*s)->TotalFlux(aocommon::Polarization::StokesI);
      sum += flux;
    }
    return sum;
  }
...
size_t NearestCluster(const std::vector<Cluster>& clusters,
...
void Output(const std::vector<Cluster>& clusters) {
  ...
  for (size_t i = 0; i != clusters.size(); ++i) {
    ...
    std::cout << "Cluster " << (i + 1) << " (" << RaDecCoord::RAToString(meanRA)
              << " " << RaDecCoord::DecToString(clusters[i].MeanDec()) << ")"
              << ": " << clusters[i].SourceCount() << " sources";
    if (clusters[i].SourceCount() > 0) {
      double sum = 0.0;
      double maxSource = clusters[i].Source(0)->TotalFlux(
                 aocommon::Polarization::StokesI),
             minSource = maxSource;
      double maxDist = 0.0;
      for (size_t s = 0; s != clusters[i].SourceCount(); ++s) {
        const ModelSource& source = *clusters[i].Source(s);
        double flux = source.TotalFlux(aocommon::Polarization::StokesI);
      ...
      std::cout << ", min=" << minSource << ", max=" << maxSource
                << ", sum=" << sum << ", max dist=" << maxDist << " deg";
      sumOfMaxDist += maxDist;
      if (maxDist > maxDistOfAll) maxDistOfAll = maxDist;
    }
    std::cout << '\n';
  }
  std::cout << "Average max dist: " << sumOfMaxDist / clusters.size()
            << " deg\n";
...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    std::cout
        << "Syntax: cluster <model-input> <model-output> <clustercount>\n";
    return -1;
  }
  Model model(argv[1]);
  ...
  for (Model::const_iterator i = model.begin(); i != model.end(); ++i) {
    if (i->ComponentCount() != 0) prunedModel.AddSource(*i);
  }
  if (model.SourceCount() != prunedModel.SourceCount()) {
    std::cout << "Removed " << model.SourceCount() - prunedModel.SourceCount()
              << " sources without components.\n";
  }
  model = prunedModel;

  std::vector<Cluster> clusters(clusterCount);
  size_t clusterIndex = 0;
  aocommon::UVector<size_t> clusterIndexPerSource(model.SourceCount());
  ...
  for (size_t cIndex = 0; cIndex != clusters.size(); ++cIndex) {
    const Cluster& cluster = clusters[cIndex];
    str << "TEXT " << cluster.MeanRA() * 180.0 / M_PI << " "
        << cluster.MeanDec() * 180.0 / M_PI << " cluster" << (cIndex + 1)
        << "\n";
  }
}
