...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    std::cout
        << "Syntax:\n\t autoprocess [options] <catalogue> <ms>\n\n"
           "Options:\n"
           "-go\n"
           "  Execute: without it, nothing will be done, only suggestions will "
           "be given.\n"
           "-noselfcal\n"
           "  Disable self cal.\n"
           "-subtract\n"
           ...
           "   Set accuracy at which to accept or immediately accept a "
           "solution.\n"
  ...
  int argi = 1;
  bool doExecute = false, doSelfCal = false, doPeel = true, doSubtract = false,
       verboseOnPolarizations = false, noPeelingRunnerupCheck = false,
       saveSolutionFiles = false;
  double minAccuracy = CalibrationMethod::DefaultMinAccuracy(),
  ...
  double subtractThreshold = 20.0, peelMinRunnerUpFactor = 3.0,
         maxCalibrateDist = 12.5;

  bool hasCorrected = ms.tableDesc().isColumn("CORRECTED_DATA");
  std::string dataColumn;
  if (hasCorrected) {
    std::cout << "This measurement set has corrected data: tasks will be "
                 "applied on the corrected data column.\n";
    dataColumn = "CORRECTED_DATA";
  } else {
    std::cout << "No corrected data in set: tasks will be applied on the data "
                 "column.\n";
    dataColumn = "DATA";
  }
  ...
  for (Model::iterator srcIter = catalogue.begin(); srcIter != catalogue.end();
       ++srcIter) {
    ModelSource& source = *srcIter;
    std::complex<double> fluxLinMatrix[4];
    double fluxStokesMatrix[4];
    for (size_t p = 0; p != 4; ++p)
      fluxStokesMatrix[p] = source.TotalFlux(
          bandData.LowestFrequency(), bandData.HighestFrequency(),
          aocommon::Polarization::IndexToStokes(p));
    aocommon::Polarization::StokesToLinear(fluxStokesMatrix, fluxLinMatrix);
    beamEvaluator.AbsToApparent(source.Peak().PosRA(), source.Peak().PosDec(),
                                fluxLinMatrix);
    double fluxStokesI =
        (fluxLinMatrix[0].real() + fluxLinMatrix[3].real()) * 0.5;
  ...
  for (size_t i = 0; i != sources.size(); ++i) {
    ...
    double distanceDeg =
        (180.0 / M_PI) * aocommon::ImageCoordinates::AngularDistance(
                             phaseCentreRA, phaseCentreDec,
                             src.second->Peak().PosRA(),
                             src.second->Peak().PosDec());
    double distanceNice = round(distanceDeg * 10.0) * 0.1,
           fluxNice = round(src.first * 10.0) * 0.1;

    std::cout << src.second->Name() << " (" << fluxNice
              << " Jy/beam, distance=" << distanceNice << " deg)\n";
    ...
    if (src.first >= peelThreshold && !peelingSourceSkipped) {
      if (distanceDeg <= maxCalibrateDist && peelSources.empty() && doSelfCal) {
        ...
        if (i + 1 < sources.size()) runnerUpFlux = sources[i + 1].first;
        if (runnerUpFlux * peelMinRunnerUpFactor > src.first &&
            !noPeelingRunnerupCheck) {
          std::cout << " -> Runner-up source " << sources[i + 1].second->Name()
                    << " is too bright for automated peeling of this source.\n";
  ...
  if (calibrateSources.empty())
    std::cout
        << "- No self-calibration (no strong sources close to the centre).\n";
  else
    std::cout << "- Self-calibrate using " << sourceList(calibrateSources)
              << " and subtract "
              << ((calibrateSources.size() > 1) ? "these sources.\n"
                                                : "this source.\n");
  ...
  if (peelSources.empty())
    ...
    if (doPeel)
      ...
      std::cout << "- Advice is to peel out " << sourceList(peelSources)
                << ", but peeling is disabled.\n";
  }
  ...
  if (subtractSources.empty())
    ...
    if (doSubtract)
      ...
      std::cout << "- Advice is to subtract " << sourceList(subtractSources)
                << ", but subtraction is disabled.\n";
  }

  std::cout << '\n';
  if (calibrateSources.size() + peelSources.size() + subtractSources.size() ==
      0)
    std::cout << "No tasks to perform.\n";
  else if (!doExecute)
    std::cout
        << "Not performing tasks: specify '-go' on command line to execute.\n";
  else {
    ...
    if (!calibrateSources.empty()) {
      ...
      Model calModel;
      for (std::vector<ModelSource*>::const_iterator i =
               calibrateSources.begin();
           i != calibrateSources.end(); ++i) {
    ...
    if (!peelSources.empty() && doPeel) {
      ...
      for (std::vector<ModelSource*>::const_iterator i = peelSources.begin();
           i != peelSources.end(); ++i) {
        Model peelModel;
        restorationModel.AddSource(**i);
        ModelSource peelSource = **i;
        std::cout << "- Source " << peelSource.Name() << "\n";
        // Correct for the beam; this is not necessarily as gains are fitted,
        // but will (a) give better initial conditions and (b) the reported
        // gains are true differential gains.
        for (ModelSource::iterator i = peelSource.begin();
             i != peelSource.end(); ++i) {
          std::complex<double> beamMatrix[4], beamGain[4];
          beamEvaluator.EvaluateAbsToApparentGain(i->PosRA(), i->PosDec(),
                                                  beamMatrix);
          aocommon::Matrix2x2::ATimesHermB(beamGain, beamMatrix, beamMatrix);
    ...
    {
            std::cout << "Spectrally subtracting " <<
    sourceList(subtractSources) << "...\n";

            Model subtractModel;
            for(std::vector<ModelSource*>::const_iterator
    src=subtractSources.begin(); src!=subtractSources.end(); ++src)
            {
                    ModelSource subtractSource = **src;
                    for(ModelSource::iterator i=subtractSource.begin();
    i!=subtractSource.end(); ++i)
                    {
                            std::complex<double> beamMatrix[4], beamGain[4];
                            beamEvaluator.EvaluateAbsToApparentGain(i->PosRA(),
    i->PosDec(), beamMatrix); Matrix2x2::ATimesHermB(beamGain, beamMatrix,
    beamMatrix); double gain = (beamGain[0].real() + beamGain[3].real()) * 0.5;
                            i->SED() *= gain;
                    }
            ...
            for(Model::const_iterator src=fittedModel.begin();
    src!=fittedModel.end(); ++src)
            {
    ...
    std::cout << "Restoration model containing "
              << restorationModel.SourceCount()
              << " sources written to model-restore.txt .\n";
  }
}
