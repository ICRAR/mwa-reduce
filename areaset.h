...
class SkyAreaElement {
 public:
  ...
  void SetToBox(double upperRA, double lowerRA, double upperDec,
                double lowerDec) {
    _type = Box;
  ...
  bool IsIn(NumType ra, NumType dec) const {
    switch (_type) {
      case Circle:
        return ImageCoordinates::AngularDistance<NumType>(
                   ra, dec, _circleCentreRA, _circleCentreDec) <= _circleRadius;
      case Box:
        return ra >= _boxLowerRA && ra <= _boxUpperRA && dec >= _boxLowerDec &&
               dec <= _boxUpperDec;
      default:
        return false;
    }
  ...
  void Save(std::ostream &stream) const {
    ...
    switch (_type) {
      case Circle:
        ...
        break;
      case Box:
        stream << "box " << RaDecCoord::RAToString(_boxUpperRA) << ' '
...
class SkyArea {
 public:
  ...
  void Save(std::ostream &stream) const {
    stream << "area {\n  name \"" << _name << "\"\n  allow-cleaning "
           << (_allowCleaning ? "True" : "False") << '\n';
    if (_weight != 0.0) stream << "weight " << _weight << '\n';
    for (std::vector<SkyAreaElement>::const_iterator i = _elements.begin();
         i != _elements.end(); ++i) {
      i->Save(stream);
    }
...
class AreaSet {
 public:
  void AddArea(const SkyArea &area) { _areas.push_back(area); }

  template <typename NumType>
  void FindAreas(std::vector<const SkyArea *> &foundAreas, NumType ra,
                 NumType dec) const {
    for (std::vector<SkyArea>::const_iterator i = _areas.begin();
         i != _areas.end(); ++i)
      if (i->IsIn(ra, dec)) foundAreas.push_back(&*i);
  }
  ...
  void FindAreasInImage(std::vector<const SkyArea *> &foundAreas, size_t x,
                        size_t y) const {
    long double l, m, ra, dec;
    ImageCoordinates::XYToLM<long double>(x, y, _pixelSizeX, _pixelSizeY,
                                          _imageWidth, _imageHeight, l, m);
    ImageCoordinates::LMToRaDec<long double>(l, m, _imageRA, _imageDec, ra,
                                             dec);
  ...
  void SetImageProperties(double pixelSizeX, double pixelSizeY, double ra,
                          double dec, size_t width, size_t height) {
