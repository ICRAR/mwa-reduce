...
class IonSolutionFile {
 public:
  ...
  IonSolutionFile() : _outputStream(0), _inputStream(0) {
    ...
    // 1: Multi-directional gain,posl,posm solutions, old format without cluster
    // descs 2: Same, but new format including cluster descs
    _header.fileType = 2;
    _header.structureType =
        0;  // ordered gain/dl/dm, polarization, channel, antenna, time
  ...
  void OpenForReading(const char* filename) {
    ...
    if (!_inputStream->good())
      throw std::runtime_error(
          "Error reading input ionospheric solutions file");
    _inputStream->read(reinterpret_cast<char*>(&_header), sizeof(_header));
    if (_header.fileType != 2)
      throw std::runtime_error(
          "Error reading ionospheric solution file; old format or file "
          "damaged");
    _clustersInFile = 0;
  }

  void ReadClusterMetaInfo(std::string& clusterName,
                           std::vector<std::string>& sourceNames) {
    clusterName = readString();
    uint32_t count;
    _inputStream->read(reinterpret_cast<char*>(&count), sizeof(uint32_t));
    sourceNames.resize(count);
    for (std::vector<std::string>::iterator i = sourceNames.begin();
         i != sourceNames.end(); ++i)
      *i = readString();
  ...
  void ReadClusterMetaInfo(
      Model& expectedSources,
    ...
    for (size_t s = 0; s != expectedSources.SourceCount(); ++s) {
      ...
      sourceNameToIndex.insert(
          std::make_pair(expectedSources.Source(s).Name(), s));
    }
    ...
    for (size_t d = 0; d != DirectionCount(); ++d) {
      ...
      for (size_t i = 0; i != sourceNames.size(); ++i) {
        std::map<std::string, size_t>::iterator sIter =
            sourceNameToIndex.find(sourceNames[i]);
        if (sIter == sourceNameToIndex.end())
          throw std::runtime_error("Source not found in model");
  ...
  double ReadSolution(IonSolutionType type, size_t interval,
                      size_t channelBlock, size_t polarization,
                      size_t direction) {
    Solution solution;
    ReadSolution(solution, interval, channelBlock, polarization, direction);
    if (!std::isfinite(solution.gain) || !std::isfinite(solution.dl) ||
        !std::isfinite(solution.dm)) {
  ...
  double ReadAverageSolution(IonSolutionType type, size_t polarization,
                             size_t direction) {
    double sum = 0.0;
    size_t count = 0;
  ...
  void ReadSolution(Solution& solution, size_t interval, size_t channelBlock,
                    size_t polarization, size_t direction) {
    if (_clustersInFile != _header.directionCount)
      throw std::runtime_error(
          "ReadSolution() called before all cluster meta data were read");
    std::unique_lock<std::mutex> lock(_mutex);
    size_t index = ((interval * _header.channelBlockCount + channelBlock) *
                        _header.polarizationCount +
                    polarization) *
                       _header.directionCount +
                   direction;
    _inputStream->seekg(_firstDataPos + sizeof(Solution) * index,
                        std::ios::beg);
    if (!_inputStream->good())
      throw std::runtime_error(
          "Error while reading solution from solutionfile: file corrupted?");
    _inputStream->read(reinterpret_cast<char*>(&solution), sizeof(Solution));
  }

  void WriteClusterMetaInfo(const std::string& clusterName,
                            const std::vector<std::string>& sourceNames) {
    writeString(clusterName);
    uint32_t count = sourceNames.size();
    _outputStream->write(reinterpret_cast<const char*>(&count),
                         sizeof(uint32_t));
    for (std::vector<std::string>::const_iterator i = sourceNames.begin();
         i != sourceNames.end(); ++i) {
      if (_writtenSourceNames.count(*i) > 0)
        throw std::runtime_error(
            "All sources in the model should have unique names!");
  ...
  void WriteSolution(const Solution& solution, size_t interval, size_t channel,
                     size_t polarization, size_t direction) {
    if (_clustersInFile != _header.directionCount)
      throw std::runtime_error(
          "WriteSolution() called before all cluster meta data were written");

    std::unique_lock<std::mutex> lock(_mutex);
    size_t index = ((interval * _header.channelBlockCount + channel) *
                        _header.polarizationCount +
                    polarization) *
                       _header.directionCount +
                   direction;
    _outputStream->seekp(_firstDataPos + sizeof(Solution) * index,
                         std::ios::beg);
    _outputStream->write(reinterpret_cast<const char*>(&solution),
                         sizeof(Solution));
  }

  void WriteChannelBlock(const Solution* solutions, size_t interval,
                         size_t channel, size_t polarization) {
    std::unique_lock<std::mutex> lock(_mutex);
    size_t index = ((interval * _header.channelBlockCount + channel) *
                        _header.polarizationCount +
                    polarization) *
    ...
    _outputStream->seekp(_firstDataPos + sizeof(Solution) * index,
                         std::ios::beg);
    _outputStream->write(reinterpret_cast<const char*>(solutions),
                         sizeof(Solution) * _header.directionCount);
  }
 ...
 private:
  ...
  void writeString(const std::string& str) {
    ...
    _outputStream->write(reinterpret_cast<const char*>(&length),
                         sizeof(uint32_t));
  ...
  std::string readString() {
    ...
    if (!_inputStream->good())
      throw std::runtime_error(
          "Error reading string length from solution file");
    aocommon::UVector<char> buffer(length + 1);
