...
RMSynthesis::RMSynthesis(const MeasuredSED& sed) : _sed(sed), _maxLSq(0.0) {}

void RMSynthesis::Synthesize() {
  MeasuredSED::const_reverse_iterator t = _sed.rbegin();
  double minLSq = BandData::FrequencyToLambda(t->first) *
                  BandData::FrequencyToLambda(t->first);
  ++t;
  double nextMinLSq = BandData::FrequencyToLambda(t->first) *
                      BandData::FrequencyToLambda(t->first);
  _maxLSq = BandData::FrequencyToLambda(_sed.begin()->first) *
            BandData::FrequencyToLambda(_sed.begin()->first);
  ...
  for (MeasuredSED::const_iterator i = _sed.begin(); i != _sed.end(); ++i) {
    double lambda = BandData::FrequencyToLambda(i->first),
           q = i->second.FluxDensity(aocommon::Polarization::StokesQ),
           u = i->second.FluxDensity(aocommon::Polarization::StokesU);
    if (std::isfinite(q) && std::isfinite(u)) addSample(lambda * lambda, q, u);
  }
