...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    std::cout
        << "Usage: peel [-datacolumn <column>] [-beam-on-source] [-mwapath "
           "<path>] [-p <phases.txt> <gains.txt>] [-pf <faraday.txt>] [-px "
           "<crossterms.txt>] [-minuv <min uvw dist>] [-a <min-accuracy> "
           "<stop-accuracy>] [-i <niter>] [-m <model>] [-scalar] [-diag] [-rhs "
           "<rhs solutions>] [-rotation] [-applybeam] [-t <solution interval "
           "timesteps>] <measurementset.ms>\n\n"
        << "This will calculate \"static\" phase offsets for all stations. It "
           "produces approximate least-squares solutions.\n";
  } else {
    int argi = 1;
    bool beamOnSource = false, applyBeam = false, onlyScalar = false,
         onlyDiag = false, onlyRotation = false;
