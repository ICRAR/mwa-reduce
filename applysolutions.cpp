...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cout << "Usage: applysolutions [-zero] [-datacolumn <name>] [-gflag "
                 "<solutions-flag-file.txt>] [-copy/-nocopy] [-s xx xy yx yy] "
                 "<ms> <gains-bin-file>\n"
                 "Will apply the found solution matrices.\n"
                 "Options:\n"
                 "  -copy/-nocopy Don't(/do) alter the original DATA column "
                 "but store the corrected data in the CORRECTED_DATA (this is "
                 "std CASA behaviour)\n"
                 "    default: -copy\n";
  } else {
    size_t argi = 1;
    ...
    if (preset) applier.SetPresets(xx, xy, yx, yy);
    applier.SetInputColumn(dataColumnName);
    if (copyData)
      applier.SetOutputColumn(casacore::MeasurementSet::columnName(
          casacore::MSMainEnums::CORRECTED_DATA));
    else
