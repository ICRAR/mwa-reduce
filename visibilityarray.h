...
class VisibilityArray {
...
#ifndef NDEBUG
    if (antenna1 == antenna2 || antenna1 >= _nAntenna ||
        antenna2 >= _nAntenna || timeIndex >= _nTimesteps) {
      std::ostringstream errMsg;
      errMsg << "Call: ValuePtr(antenna1=" << antenna1
             << ", antenna2=" << antenna2 << ", timeIndex=" << timeIndex
             << ") - ";
      ...
      if (antenna1 >= _nAntenna)
        throw std::runtime_error(
            s + "Requested index value for antenna1 out of bounds");
      if (antenna2 >= _nAntenna)
        throw std::runtime_error(
            s + "Requested index value for antenna2 out of bounds");
      if (timeIndex >= _nTimesteps)
        throw std::runtime_error(
            s + "Requested index value for time index out of bounds");
    }
#endif
    if (antenna1 > antenna2) std::swap(antenna1, antenna2);
    return &_values[NPol * _nChannels *
                    (BaselineIndex(antenna1, antenna2) +
                     timeIndex * _nBaselines)];
  }
 ...
 private:
  ...
  // Ordered in Polarization, Frequency, Antenna2, Antenna1 (i.e., Antenna1 is
  // most significant, and a1 <= a2), Time
