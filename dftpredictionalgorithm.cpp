...
DFTPredictionImage::DFTPredictionImage(size_t width, size_t height,
  ...
  for (size_t p = 0; p != 4; ++p) {
    ...
    for (size_t i = 0; i != s; ++i) _images[p][i] = 0.0;
  }
}

void DFTPredictionImage::Add(PolarizationEnum polarization,
                             const double* image) {
...
void DFTPredictionImage::Add(PolarizationEnum polarization, const double* real,
...
void DFTPredictionImage::FindComponents(DFTPredictionInput& destination,
  ...
  for (size_t y = 0; y != _height; ++y) {
    for (size_t x = 0; x != _width; ++x) {
      if (_images[0][index] != 0.0 || _images[1][index] != 0.0 ||
        ...
        double l, m;
        ImageCoordinates::XYToLM(x, y, pixelSizeX, pixelSizeY, _width, _height,
                                 l, m);
        l += dl;
        m += dm;
        double ra, dec;
        ImageCoordinates::LMToRaDec(l, m, phaseCentreRA, phaseCentreDec, ra,
                                    dec);
        double flux[4] = {_images[0][index], _images[1][index],
                          _images[2][index], _images[3][index]};
        destination.AddComponent(
            DFTPredictionComponent(ra, dec, l, m, flux, channelCount));
      }
      ++index;
    }
  }
...
void DFTPredictionAlgorithm::Predict(MC2x2& dest, double u, double v, double w,
                                     size_t channelIndex, size_t a1,
                                     size_t a2) {
  MC2x2 single;
...
void DFTPredictionAlgorithm::predict(MC2x2& dest, double u, double v, double w,
                                     size_t channelIndex, size_t a1, size_t a2,
                                     DFTPredictionComponent& component) {
  ...
  MC2x2 temp, appFlux;
  MC2x2::ATimesB(temp, component.AntennaInfo(a1).BeamValue(channelIndex),
                 component.LinearFlux(channelIndex));
  MC2x2::ATimesHermB(appFlux, temp,
                     component.AntennaInfo(a2).BeamValue(channelIndex));
  if (component.IsGaussian()) {
...
void DFTPredictionAlgorithm::UpdateBeam(LBeamEvaluator& beamEvaluator) {
  for (DFTPredictionComponent& component : _input) {
    ...
    beamEvaluator.PrecalculatePositionInfo(posInfo, component.RA(),
                                           component.Dec());

    for (size_t antenna = 0; antenna != component.AntennaCount(); ++antenna) {
      DFTAntennaInfo& antennaInfo = component.AntennaInfo(antenna);

      for (size_t channel = 0; channel != antennaInfo.ChannelCount();
           ++channel) {
        double freq = _band.ChannelFrequency(channel);
        beamEvaluator.Evaluate(posInfo, freq, antenna,
                               antennaInfo.BeamValue(channel));
