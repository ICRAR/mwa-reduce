...
class ImageBufferAllocator {
 public:
  ...
  ~ImageBufferAllocator() {
    ...
    if (usedCount != 0) {
      str << usedCount
          << " image buffer(s) were still in use when image buffer allocator "
             "was destroyed!";
  ...
  void ReportStatistics() const {
    ...
    for (typename std::vector<Buffer>::const_iterator i = _buffers.begin();
         i != _buffers.end(); ++i) {
    ...
    if (buffer != 0) {
      ...
      if (!found)
        throw std::runtime_error(
            "Invalid or double call to ImageBufferAllocator::Free(NumType*).");
      --_nReal;
    }
  }
    ...
    if (buffer != 0) {
      ...
      if (!found)
        throw std::runtime_error(
            ...
            "ImageBufferAllocator::Free(std::complex<NumType>*).");
      --_nComplex;
    }
  }
 ...
 private:
  ...
  Buffer* allocateNewBuffer(size_t size) {
    ...
    int errVal =
        posix_memalign(reinterpret_cast<void**>(&buffer->ptr),
                       sizeof(NumType) * 2, size * sizeof(NumType) * 2);
    if (errVal != 0) {
      switch (errVal) {
        case EINVAL:
          throw std::runtime_error(
              "posix_memalign() failed: the alignment argument was not a power "
              "of two, or was not a multiple of sizeof(void *)");
        case ENOMEM:
          throw std::runtime_error(
              "posix_memalign() failed: there was insufficient memory to "
              "fulfill the allocation request.");
        default:
          throw std::runtime_error(
              "posix_memalign() failed but returned unknown error value");
      }
    }
    buffer->size = size;
...
class ImageBufferAllocator {
 public:
  ...
  NumType* Allocate(size_t size) { return new NumType[size]; }

  std::complex<NumType>* AllocateComplex(size_t size) {
    return new std::complex<NumType>[size];
  }

  void Free(NumType* buffer) { delete[] buffer; }

  void Free(std::complex<NumType>* buffer) { delete[] buffer; }
};
