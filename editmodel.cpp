...
void readImage(aocommon::UVector<double>& image, FitsReader& templateReader,
...
int main(int argc, char* argv[]) {
  if (argc == 1) {
    std::cout
        << "editmodel -- Interpolation, extrapolation, plotting and scaling of "
           "the \n"
           "spectral energy distribution. Usage:\n"
           "\teditmodel [-p [-ft]] [-rmp] [-m <output model>] [-o] [-s "
           "<scale>] [-s-to <freq> <flux>] [-sp <peakflux A> <freq A> "
           "<peakflux B> <freq B>] [-sc <intflux A> <freq A> <intflux B> <freq "
           "B>] [-set0/1/2/3 <flux>] [-unpolarized] [-pl] [-t <threshold>] "
           "[-tbeam <beamprefix> <threshold-ratio>] [-tc <compthreshold>] "
           "[-tcl <cluster threshold] [-r <new-nr-channels>] [-ravg "
           "<new-nr-channels>] [-near/outside <ra> <dec> <dist>] "
           "[-combine-diff-meas] [-collect <name>] [-uncollect] [-rnd <n> <ra> "
           "<dec> <dist>] [-sort] [-sortbeam <beamprefix>] [-lognlogs "
           "<frequency> <bincount>] [-stats] [-setfrequency <val>] [-delnans] "
           "[-sagecal <prefix> <chunksize>] [-dppp-model <filename>] "
           "[-skymodel <filename>] [-toapp <beamprefix>] [-save-clusters "
           "<clusters.ann>] [-list] [-evaluate <freq>] [-scale-to <model> "
           "<freq-start> <freq-end> <terms>] [-select <name>] [-search <count> "
           "<ra> <dec>] [-simuniform N RA Dec dist flux] [-simpopulation RA "
           "Dec dist lowflux highflux] [-min-separation <angle>] [-replace-si "
           "<n> <si> <terms...>] [-set-cluster <name>] "
           "[-rts <filename>] [-to-powerlaw <n>] [-shift <ra-angle> "
           "<dec-angle>] [-kvis <kvis .ann file>] [-split/-split2 <ra1> <dec1> "
           "<ra2> <dec2>] "
           "<model> [<more models...>]\n";
    return 0;
  }
  int argi = 1;
  bool outputPlot = false, doFT = false, outputRMPlot = false,
       outputCsv = false, outputSICsv = false, powerlaw = false,
       applyThreshold = false, applyIndexThreshold = false,
       applyComponentThreshold = false, applyClusterThreshold = false,
       resample = false, resampleByAveraging = false, unpolarized = false,
       delNaNs = false, outputList = false;
  double evaluateFrequency = 0.0;
  bool setPolarization[4] = {false, false, false, false};
  long double setPolFlux[4] = {0.0, 0.0, 0.0, 0.0};
  long double scale = 1.0, scaleToFreq = 0.0, scaleToFlux = 0.0,
              threshold = 0.0, logNlogSFrequency = 0.0;
  long double scalePeakA = 1.0, scaleFreqA = 0.0, scalePeakB = 1.0,
              scaleFreqB = 0.0;
  size_t newChannelCount = 0, logNlogSBinCount = 0,
         sagecalFirstClusterChunkSize = 1;
  std::string outputModel, collectName, csvFilename, plotTitle, sagecalPrefix,
      toAppBeamPrefix, dpppModelFilename, kvisAnnFile, sortBeamPrefix,
      tBeamPrefix;
  bool nearFilter = false, outsideFilter = false, scalePeak = false,
       scaleSource = false, scaleWithSI = false, doCollect = false,
       doSort = false, setFrequency = false, uncollect = false;
  long double nearFilterRA = 0.0, nearFilterDec = 0.0, nearFilterDist = 0.0,
              setFrequencyValue = 0.0, powerSumLimit = 0.0, shiftRA = 0.0,
              shiftDec = 0.0;
  enum {
    AddFluxes,
    AverageFluxes,
    DifferentFrequencies
  } combineStrategy = AddFluxes;
  ...
  double simUniformRa = 0.0, simUniformDec = 0.0, simUniformDist = 0.0,
         simUniformFlux = 0.0;
  double simPopulationRa = 0.0, simPopulationDec = 0.0, simPopulationDist = 0.0,
         simPopulationFluxLo = 0.0, simPopulationFluxHi = 0.0;
  ...
  aocommon::UVector<double> replaceSIvalues;
  ...
  while (argi != argc && argv[argi][0] == '-') {
    ...
    if (option == "p") {
      ...
      ++argi;
      nearFilterDist =
          Angle::Parse(argv[argi], "near filter distance", Angle::kDegrees);
      nearFilter = true;
      ...
      ++argi;
      nearFilterDist =
          Angle::Parse(argv[argi], "outside filter distance", Angle::kDegrees);
      outsideFilter = true;
    } else if (option == "rnd") {
      ++argi;
      rndN = atof(argv[argi]);
      ...
      ++argi;
      rndDist = Angle::Parse(argv[argi], "rnd", Angle::kDegrees);
    ...
    } else if (option == "sp")  // scale w.r.t. peak flux
    {
      scalePeak = true;
      ++argi;
      scalePeakA = atof(argv[argi]);
      ++argi;
      scaleFreqA = atof(argv[argi]) * 1000000.0;
      ++argi;
      scalePeakB = atof(argv[argi]);
      ++argi;
      scaleFreqB = atof(argv[argi]) * 1000000.0;
    ...
    } else if (option == "sc-si")  // scale w.r.t. whole source flux
    {
      ...
      ++argi;
      minSeparation =
          Angle::Parse(argv[argi], "min-separation", Angle::kArcseconds);
      ...
      shiftRA = Angle::Parse(argv[argi + 1], "shift RA", Angle::kDegrees);
      shiftDec = Angle::Parse(argv[argi + 2], "shift Dec", Angle::kDegrees);
      argi += 2;
  ...
  if (combineStrategy == AverageFluxes) {
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
         ++sourcePtr) {
      for (ModelSource::iterator compPtr = sourcePtr->begin();
        ...
        for (MeasuredSED::iterator m = sed.begin(); m != sed.end(); ++m) {
          for (size_t p = 0; p != 4; ++p)
            m->second.SetFluxDensityFromIndex(
                p, m->second.FluxDensityFromIndex(p) * fact);
        }
      }
    }
  }
  if (scaleToNTerms > 0) {
    ...
    if (scaleModel.SourceCount() != 1)
      throw std::runtime_error(
          "Scale reference model should have a single source");
    const ModelSource& source = scaleModel.Source(0);
    aocommon::UVector<double> frequencies(scaleToNTerms);
    aocommon::UVector<float> values(scaleToNTerms);
    aocommon::UVector<float> weights(scaleToNTerms, 1.0);
    ...
    for (size_t term = 0; term != scaleToNTerms; ++term) {
      double frequency = scaleToFreqStart + scaleToBandwidth * double(term) /
                                                double(scaleToNTerms - 1);
      values[term] =
          source.TotalFlux(frequency, aocommon::Polarization::StokesI);
      frequencies[term] = frequency;
      std::cout << round(frequency * 1e-6) << " MHz\t" << values[term]
                << " Jy\t"
                << model.Source(0).TotalFlux(frequency,
                                             aocommon::Polarization::StokesI)
                << " Jy\n";
    }

    for (ModelSource& source : model) {
      aocommon::UVector<double> scalingFactors(values.size());
      for (size_t term = 0; term != scaleToNTerms; ++term) {
        double frequency = frequencies[term];
        double flux = values[term];
        scalingFactors[term] =
            flux / source.TotalFlux(frequency, aocommon::Polarization::StokesI);
      }
      for (ModelComponent& component : source) {
        aocommon::UVector<float> fitValues(scaleToNTerms);
        for (size_t term = 0; term != scaleToNTerms; ++term) {
          double frequency = frequencies[term];
          double oldFlux = component.SED().FluxAtFrequency(
              frequency, aocommon::Polarization::StokesI);
        ...
        if (sed.IsLogarithmic())
          mode = SpectralFittingMode::LogPolynomial;
        else
          mode = SpectralFittingMode::Polynomial;
        size_t nTerms = sed.NTerms();
        SpectralFitter fitter(mode, nTerms);
        fitter.SetFrequencies(frequencies.data(), weights.data(),
                              scaleToNTerms);
        aocommon::UVector<float> newTerms;
        fitter.Fit(newTerms, fitValues.data(), 0, 0);
        sed.SetFromStokesIFit(refFreq, aocommon::UVector<double>(
    ...
    for (ModelSource& source : model) {
      for (ModelComponent& component : source) {
        if (component.HasPowerLawSED()) {
          ...
          throw std::runtime_error(
              "Can only set SI of sources with a power law SED");
      }
    }
  }
  ...
  if (simUniformN != 0 || simPopulationFluxHi != 0.0) {
    ...
    if (simUniformN != 0) {
      ...
      simN = round(area * popFac / (popExp + 1.0) *
                   (pow(simPopulationFluxHi, (popExp + 1.0)) -
                    pow(simPopulationFluxLo, (popExp + 1.0))));
      std::cout << "N = " << simN << "\n";
      samplePL = true;
    }
    ...
    for (size_t i = 0; i != simN; ++i) {
      ...
      } while (ImageCoordinates::AngularDistance(ra, dec, simRa, simDec) >
               simDist);
      ModelComponent comp;
      comp.SetPosRA(ra);
      comp.SetPosDec(dec);
      ...
      if (samplePL) {
        ...
        flux =
            pow((pow(simPopulationFluxHi, popExp + 1.0) - loTerm) * u + loTerm,
                1.0 / (popExp + 1.0));
      ...
      aocommon::UVector<double> si(1, spectralIndex);
  ...
  if (delNaNs) {
    for (Model::iterator s = model.begin(); s != model.end(); ++s) {
      for (ModelSource::iterator c = s->begin(); c != s->end(); ++c) {
        if (c->HasMeasuredSED()) c->MSED().RemoveInvalidMeasurements();
      }
    }
  }
  if (doSort) model.Sort();
  if (!sortBeamPrefix.empty()) {
    std::vector<aocommon::UVector<double>> images(8);
    ...
    size_t width = templateReader.ImageWidth(),
           height = templateReader.ImageHeight();
    ...
    for (size_t i = 0; i != model.SourceCount(); ++i) {
      ...
      for (ModelSource::iterator compPtr = model.Source(i).begin();
           compPtr != model.Source(i).end(); ++compPtr) {
        ...
        ImageCoordinates::RaDecToLM<long double>(
            compPtr->PosRA(), compPtr->PosDec(), templateReader.PhaseCentreRA(),
            templateReader.PhaseCentreDec(), l, m);
        ImageCoordinates::LMToXY<long double>(l, m, templateReader.PixelSizeX(),
                                              templateReader.PixelSizeY(),
                                              width, height, x, y);
        ...
        for (size_t p = 0; p != 4; ++p) {
          beamValues[p] = std::complex<double>(
              images[p * 2][y * height + x], images[p * 2 + 1][y * height + x]);
        }
        std::complex<double> linearFlux[4], tempValues[4];
        aocommon::Polarization::StokesToLinear(stokesFlux, linearFlux);
        Matrix2x2::ATimesB(tempValues, beamValues, linearFlux);
        Matrix2x2::ATimesHermB(linearFlux, tempValues, beamValues);
        aocommon::Polarization::LinearToStokes(linearFlux, stokesFlux);
  ...
  if (doSearch) {
    ...
    for (size_t i = 0; i != model.SourceCount(); ++i) {
      ...
      double dist =
          ImageCoordinates::AngularDistance(ra, dec, searchRA, searchDec);
      sortedSources.insert(
          std::pair<double, const ModelSource*>(dist, &source));
    }
    Model newModel;
    size_t count = 0;
    for (std::map<double, const ModelSource*>::const_iterator i =
             sortedSources.begin();
         i != sortedSources.end() && count < searchCount; ++i, ++count) {
  ...
  if (!toAppBeamPrefix.empty()) {
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
         ++sourcePtr) {
      std::cout << sourcePtr->Name() << ": "
                << sourcePtr->TotalFlux(refFreq,
                                        aocommon::Polarization::StokesI)
                << " -> ";
      for (ModelSource::iterator compPtr = sourcePtr->begin();
           compPtr != sourcePtr->end(); ++compPtr) {
        ...
        aocommon::Polarization::LinearToStokes(linearFlux, stokesFlux);
        PowerLawSED plSED;
        aocommon::UVector<double> siTerms;
        plSED.SetData(refFreq, stokesFlux, siTerms);
        compPtr->SetSED(plSED);
      }
      std::cout << sourcePtr->TotalFlux(refFreq,
                                        aocommon::Polarization::StokesI)
                << '\n';
    }
  }
  ...
  if (applyThreshold) {
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
         ++sourcePtr) {
      double totalFlux = sourcePtr->TotalFlux(aocommon::Polarization::StokesI);
      if (totalFlux >= threshold) temp.AddSource(*sourcePtr);
    }
    model = temp;
  }
  ...
  if (applyComponentThreshold) {
    Model temp;
    for (Model::const_iterator sourcePtr = model.begin();
         sourcePtr != model.end(); ++sourcePtr) {
      ModelSource newSource = *sourcePtr;
      newSource.ClearComponents();
      for (ModelSource::const_iterator compPtr = sourcePtr->begin();
           compPtr != sourcePtr->end(); ++compPtr) {
        if (compPtr->HasSED()) {
          const SpectralEnergyDistribution& sed = compPtr->SED();
          long double refFreq = sed.ReferenceFrequencyHz();
          long double flux =
              sed.FluxAtFrequency(refFreq, aocommon::Polarization::StokesI);
    ...
    for (size_t i = 0; i != model.SourceCount(); ++i) {
      ...
      for (size_t j = 0; j != model.SourceCount(); ++j) {
        if (i != j) {
          ...
          double dist = ImageCoordinates::AngularDistance(
              s1.MeanRA(), s1.MeanDec(), s2.MeanRA(), s2.MeanDec());
          if (dist < minSeparation) {
            if (s1.TotalFlux(aocommon::Polarization::StokesI) <
                s2.TotalFlux(aocommon::Polarization::StokesI))
              remain = false;
          }
        }
      }
      if (remain) newModel.AddSource(s1);
    }
    model = newModel;
  }
  ...
  if (applyClusterThreshold) {
    ...
    for (std::vector<std::string>::const_iterator clName = clusterNames.begin();
         clName != clusterNames.end(); ++clName) {
      SourceGroup group;
      model.GetSourcesInCluster(*clName, group);

      if (std::fabs(group.TotalFlux(aocommon::Polarization::StokesI)) >=
          threshold) {
        for (SourceGroup::const_iterator s = group.begin(); s != group.end();
             ++s)
          temp.AddSource(*s);
      } else
        ++thresholded;
    ...
    model = temp;
    std::cout << "Thresholded " << thresholded << " / " << total
              << " clusters.\n";
  }
  ...
  if (rndN != 0) {
    for (size_t i = 0; i != rndN; ++i) {
      ...
      } while (ImageCoordinates::AngularDistance(rndRA, rndDec, ra, dec) >=
               rndDist);
  ...
  if (nearFilter || outsideFilter) {
    ...
    for (size_t i = model.SourceCount(); i > 0; --i) {
      ...
      if (source.ComponentCount() > 0) {
        double dist = ImageCoordinates::AngularDistance(
            source.Peak().PosRA(), source.Peak().PosDec(), nearFilterRA,
            nearFilterDec);
  ...
  if (!setClusterName.empty()) {
    for (ModelSource& source : model) source.SetClusterName(setClusterName);
  }
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
      ...
      for (ModelSource::iterator compPtr = sourcePtr->begin();
        ...
        if (powerlaw) {
          ...
          for (size_t p = 0; p != 4; ++p) {
            sed.FitPowerlaw(f, e, aocommon::Polarization::IndexToStokes(p));
          ...
          MeasuredSED newSED;
          const long double newBandSize =
              (endFreq - startFreq) / newChannelCount;
          for (size_t newChIndex = 0; newChIndex != newChannelCount;
               ++newChIndex) {
            long double chStartFreq = startFreq + newBandSize * newChIndex;
            long double chEndFreq =
                startFreq + newBandSize * (newChIndex + 1.0);
            ...
            if (newChannelCount >= sed.MeasurementCount())
              flux = sed.FluxAtFrequency((chStartFreq + chEndFreq) * 0.5,
                                         aocommon::Polarization::StokesI);
            else if (resampleByAveraging)
              flux = sed.AverageFlux(chStartFreq, chEndFreq,
                                     aocommon::Polarization::StokesI);
            else
              flux = sed.IntegratedFlux(chStartFreq, chEndFreq,
                                        aocommon::Polarization::StokesI);
            m.SetFluxDensity(aocommon::Polarization::StokesI, flux);

            aocommon::PolarizationEnum pols[3] = {
  ...
  if (scaleToFreq != 0.0) {
    ...
    for (ModelSource& source : model) {
      double flux =
          source.TotalFlux(scaleToFreq, aocommon::Polarization::StokesI);
  ...
  if (scalePeak || scaleSource) {
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
      ...
      for (size_t p = 0; p != 4; ++p) {
        if (scalePeak) {
          long double oldFluxA = sourcePtr->Peak().SED().FluxAtFrequency(
                          scaleFreqA, aocommon::Polarization::IndexToStokes(p)),
                      oldFluxB = sourcePtr->Peak().SED().FluxAtFrequency(
                          scaleFreqB, aocommon::Polarization::IndexToStokes(p));
          factorA[p] = oldFluxA == 0.0 ? 0.0 : scalePeakA / oldFluxA;
          factorB[p] = oldFluxB == 0.0 ? 0.0 : scalePeakB / oldFluxB;
        } else {
          long double oldFluxA = sourcePtr->TotalFlux(
                          scaleFreqA, aocommon::Polarization::IndexToStokes(p)),
                      oldFluxB = sourcePtr->TotalFlux(
                          scaleFreqB, aocommon::Polarization::IndexToStokes(p));
          factorA[p] = oldFluxA == 0.0 ? 0.0 : scalePeakA / oldFluxA;
          factorB[p] = oldFluxB == 0.0 ? 0.0 : scalePeakB / oldFluxB;
        }
      }
      std::cout << "Scale factor for " << sourcePtr->Name() << ": "
                << factorA[0];
      ...
      if (scaleWithSI) {
        ...
        aocommon::UVector<double> sis(1, si);
        double freq = 0.5 * (scaleFreqA + scaleFreqB);
        for (ModelSource::iterator compPtr = sourcePtr->begin();
             compPtr != sourcePtr->end(); ++compPtr) {
          double brightnessVec[4];
          for (size_t p = 0; p != 4; ++p) {
            long double oldFluxA = compPtr->SED().FluxAtFrequency(
                scaleFreqA, aocommon::Polarization::IndexToStokes(p));
            long double oldFluxB = compPtr->SED().FluxAtFrequency(
                scaleFreqB, aocommon::Polarization::IndexToStokes(p));
    ...
    for (const ModelSource& source : model) {
      ...
      for (const ModelComponent& comp : source) {
        if (comp.HasMeasuredSED()) {
          ...
          sed.FitLogPolynomial(terms, toPowerlaw,
                               aocommon::Polarization::StokesI, freq);

          PowerLawSED plSed;
          plSed.SetFromStokesIFit(
              freq, aocommon::UVector<double>(terms.begin(), terms.end()));
  ...
  if (minMeasurements != 0) {
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
      ...
      if (isValid)
        ...
        std::cout << "Removing source " << sourcePtr->Name()
                  << ": a component has only " << measCount
                  << " measurements.\n";
      }
    }
    model = std::move(newModel);
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
      ...
      for (ModelSource::iterator compPtr = sourcePtr->begin();
        ...
        if (sed.MeasurementCount() != 1)
          throw std::runtime_error(
              "setfrequency requires one measurement per component");
  ...
  if (sourceStats) {
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
      ...
      double stokesI =
          source.TotalFlux(150000000.0, aocommon::Polarization::StokesI);
      sum += stokesI;
  ...
  if (logNlogS) {
    ...
    for (Model::iterator sourcePtr = model.begin(); sourcePtr != model.end();
         ++sourcePtr) {
      const ModelSource& source = *sourcePtr;
      double v = source.TotalFlux(logNlogSFrequency * 1000000.0,
                                  aocommon::Polarization::StokesI);
      histogram.Add(v);
    }
    long double max = histogram.MaxAmplitude(),
                min = histogram.MinPositiveAmplitude();
    long double step = powl(max / min, 1.0 / logNlogSBinCount);
    std::ofstream plotDataStream("lognlogs-data.txt");
    aocommon::UVector<long double> vals(logNlogSBinCount);
    for (size_t i = 0; i != logNlogSBinCount; ++i) {
      long double intervalStart = min * powl(step, i),
      ...
      plotDataStream << intervalStart << '\t' << intervalEnd << '\t'
                     << intervalStart * sqrt(step) << '\t' << val << '\t'
                     << valNormalized << '\n';
    ...
    plotStream << "set terminal postscript enhanced color\n"
               ...
               << "]\n"
                  ...
                  "plot \"lognlogs-data.txt\" using 2:(column(5)/"
               << avg
               << ") with lines lw 2.0 title \"Measured\", \\\n"
  ...
  if (powerSumLimit != 0.0) {
    ...
    for (const ModelSource& s : sorted) {
      ...
      long double f = sed.FluxAtFrequency(sed.ReferenceFrequencyHz(),
                                          aocommon::Polarization::StokesI);
      sum += f;
      powerSum += f * f;
      if (!limitReached && powerSum >= powerSumLimit) {
        limitReached = true;
        std::cout << count << " sources required to reach power sum limit of "
                  << powerSumLimit << " Jy^2.\n";
      }
    }
  ...
  if (!dpppModelFilename.empty()) {
    lofartools::SaveDp3Model(dpppModelFilename, model, true);
  }

  if (!kvisAnnFile.empty()) {
    std::ofstream str(kvisAnnFile);
    for (const ModelSource& source : model) {
      str << "TEXT " << source.MeanRA() * 180.0 / M_PI << " "
          << source.MeanDec() * 180.0 / M_PI << " " << source.Name() << "\n";
    }
  }
  ...
  if (!sagecalPrefix.empty()) {
    ...
    sagecalSourceFile << "## name h m s d m s I Q U V si0 si1 si2 RM eX(rad) "
    ...
    for (Model::const_iterator s = model.begin(); s != model.end(); ++s) {
      ...
      for (size_t compIndex = 0; compIndex != s->ComponentCount();
        ...
        // Note that Sagecal uses 2xsigma, where sigma is the Gaussian width
        // parameter, in contrast to the FWHM that is mostly used. This is
        // converted here:
        long double axMaj =
            2.0 * c.MajorAxis() / (2.0L * std::sqrt(2.0L * std::log(2.0L)));
        long double axMin =
            2.0 * c.MinorAxis() / (2.0L * std::sqrt(2.0L * std::log(2.0L)));
        ...
        sagecalSourceFile << cName << ' ' << raH << ' ' << raM << ' ' << raS
                          << ' ' << decD << ' ' << decM << ' ' << decS << ' '
                          << iquv[0] << ' ' << iquv[1] << ' ' << iquv[2] << ' '
                          << iquv[3] << ' ' << si[0] << ' ' << si[1] / M_LN10
                          << ' ' << si[2] / (M_LN10 * M_LN10) << ' ' << 0.0
                          << ' '  // rm
                          << axMaj << ' ' << axMin << ' ' << axPA << ' '
                          << refFreq << '\n';
      }
    ...
    for (Model::const_iterator s = model.begin(); s != model.end(); ++s) {
      ...
      if (clustersAdded.count(cName) == 0) {
        ...
        for (std::vector<const ModelSource*>::const_iterator s =
                 sources.begin();
             s != sources.end(); ++s) {
          for (size_t compIndex = 0; compIndex != (*s)->ComponentCount();
               ++compIndex) {
            const ModelComponent& c = (*s)->Component(compIndex);
            sagecalClusterFile << ' '
                               << getSagecalSourceName(**s, c, compIndex);
          }
        }
        sagecalClusterFile << '\n';
        ++clusterId;
  ...
  if (outputCsv) {
    ...
    if (model.SourceCount() != 0) {
      if (model.SourceCount() != 1)
        std::cout << "Warning: multiple sources in model, but will only output "
                     "a csv file for the first source.\n";
      const ModelSource& source = *model.begin();
      if (source.ComponentCount() != 1)
        std::cout << "Warning: first source has multiple components; only "
                     "outputting first component.\n";
      if (source.begin()->HasMeasuredSED()) {
        ...
        for (MeasuredSED::const_iterator iter = sed.begin(); iter != sed.end();
             ++iter) {
          const Measurement& m = iter->second;
          long double i = m.FluxDensity(aocommon::Polarization::StokesI),
                      q = m.FluxDensity(aocommon::Polarization::StokesQ),
                      u = m.FluxDensity(aocommon::Polarization::StokesU),
                      v = m.FluxDensity(aocommon::Polarization::StokesV);
          if (std::isfinite(i) && std::isfinite(q) && std::isfinite(u) &&
              std::isfinite(v)) {
            csvFile << m.FrequencyHz() * 1e-6 << ',' << i << ',' << q << ','
                    << u << ',' << v << '\n';
          }
        }
      }
    ...
    if (model.SourceCount() != 0) {
      for (Model::const_iterator s = model.begin(); s != model.end(); ++s) {
        ...
        if (source.begin()->HasMeasuredSED()) {
          ...
          double flux = sed.AverageFlux(aocommon::Polarization::StokesI);
          long double f, si;
          sed.FitPowerlaw(f, si, aocommon::Polarization::StokesI);

          csvFile << source.MeanRA() * 180.0 / M_PI << ','
                  << source.MeanDec() * 180.0 / M_PI << ',' << flux << ',' << si
                  << '\n';
        }
      }
    }
  ...
  if (outputRMPlot) {
    ...
    for (Model::const_iterator sourcePtr = model.begin();
      ...
      for (ModelSource::const_iterator compPtr = sourcePtr->begin();
        ...
        plotStream << "\"" << dataStreamName.str()
                   << "\" using 1:2 with lines lw 1.0 title \"\"\\\n";
        ...
        rmSynth.Synthesize();
        const aocommon::UVector<double>& fdf = rmSynth.FDF();

        for (size_t i = 0; i != fdf.size(); ++i) {
          dataStream << rmSynth.IndexToValue(i) << '\t' << std::abs(fdf[i])
                     << '\n';
        }
        ++compIndex;
      }
  ...
  if (outputPlot) {
    ...
    if (!plotTitle.empty()) plotFT.SetTitle(plotTitle);
    ...
    for (Model::const_iterator sourcePtr = model.begin();
      ...
      for (ModelSource::const_iterator compPtr = sourcePtr->begin();
        ...
        GNUPlot::Line* lineI =
            plot.AddLine(dataStreamName.str() + "I.txt", "I");
        GNUPlot::Line* lineQ =
            plot.AddLine(dataStreamName.str() + "Q.txt", "Q");
        GNUPlot::Line* lineU =
            plot.AddLine(dataStreamName.str() + "U.txt", "U");
        GNUPlot::Line* lineV =
            plot.AddLine(dataStreamName.str() + "V.txt", "V");
        ...
        if (doFT)
          lineFTdelay =
              plotFTdelay.AddLine(dataStreamName.str() + "FT-delay.txt", "");
        ...
        if (sourceIndex < 10 && compPtr == sourcePtr->begin()) {
          plot10I.AddLineFromExistingFile(dataStreamName.str() + "I.txt",
                                          sourcePtr->Name());
        }
        ...
        if (compPtr->HasMeasuredSED()) {
          ...
          long double e, f;
          sed.FitPowerlaw(f, e, aocommon::Polarization::StokesI);
          std::ostringstream funcStr;
          funcStr << f << " * (x*1000000)**" << e;
          plotIFit.AddFunction(funcStr.str());
          aocommon::UVector<float> terms;
          sed.FitLogPolynomial(terms, 3, aocommon::Polarization::StokesI,
                               sed.ReferenceFrequencyHz());
          std::ostringstream func2Str;
          func2Str << terms[0] << "* exp(" << terms[1] << "*log(x/"
                   << sed.ReferenceFrequencyHz() << "*1000000) + " << terms[2]
                   << "*log(x/" << sed.ReferenceFrequencyHz()
                   << "*1000000)**2)";
          plotIFit.AddFunction(func2Str.str());

          if (doFT) {
            SpectrumFT sft(sed.MeasurementCount());
            aocommon::UVector<double> power;
          ...
          for (std::vector<Measurement>::const_iterator i =
                   measurements.begin();
               i != measurements.end(); ++i) {
            lineI->AddPoint(i->FrequencyHz() / 1000000.0,
                            i->FluxDensity(aocommon::Polarization::StokesI));
            lineQ->AddPoint(i->FrequencyHz() / 1000000.0,
                            i->FluxDensity(aocommon::Polarization::StokesQ));
            lineU->AddPoint(i->FrequencyHz() / 1000000.0,
                            i->FluxDensity(aocommon::Polarization::StokesU));
            lineV->AddPoint(i->FrequencyHz() / 1000000.0,
                            i->FluxDensity(aocommon::Polarization::StokesV));
          }
          ++compIndex;
        }
      }
  ...
  if (!saveClusters.empty()) {
    ...
    for (const std::string& cluster : clusters) {
      ...
      if (!std::isfinite(ra) || !std::isfinite(dec))
        throw std::runtime_error(
            "One of the directions has a non-finite RA or Dec");
  ...
  if (outputList) {
    for (Model::const_iterator s = model.begin(); s != model.end(); ++s) {
      ...
      std::cout << source.Name() << " "
                ...
                << source.TotalFlux(aocommon::Polarization::StokesI) << '\n';
    }
  }
  ...
  if (!rtsFilename.empty()) {
    ...
    for (const ModelSource& s : model) {
      ...
      rtsStream << "SOURCE "
                ...
                << name << " " << compIter->PosRA() / M_PI * 12.0 << " "
                << compIter->PosDec() / M_PI * 180.0 << '\n';
      if (compIter->Type() == ModelComponent::GaussianSource) {
        double rtsMaj = sqrt(M_PI * M_PI / (2.0 * M_LN2)) *
                        compIter->MajorAxis() * (180.0 * 60.0 / M_PI),
               rtsMin = sqrt(M_PI * M_PI / (2.0 * M_LN2)) *
                        compIter->MinorAxis() * (180.0 * 60.0 / M_PI),
               rtsRot = compIter->PositionAngle() * (180.0 / M_PI);
        rtsStream << "GAUSSIAN " << rtsRot << ' ' << rtsMaj << ' ' << rtsMin
                  << '\n';
      }
      rtsStream << "FREQ 150.0e6 "
                << compIter->SED().FluxAtFrequency(
                       150e6, aocommon::Polarization::StokesI)
                << " 0 0 0\n"
                << "FREQ 200.0e6 "
                << compIter->SED().FluxAtFrequency(
                       200e6, aocommon::Polarization::StokesI)
                << " 0 0 0\n";
      ++compIter;
      while (compIter != s.end()) {
        rtsStream << "COMPONENT " << compIter->PosRA() / M_PI * 12.0 << " "
                  << compIter->PosDec() / M_PI * 180.0 << '\n';
        if (compIter->Type() == ModelComponent::GaussianSource) {
          double rtsMaj = sqrt(M_PI * M_PI / (2.0 * M_LN2)) *
                          compIter->MajorAxis() * (180.0 * 60.0 / M_PI),
  ...
  if (evaluateFrequency != 0.0) {
    for (Model::const_iterator s = model.begin(); s != model.end(); ++s) {
      ...
      std::cout << source.Name() << " "
                ...
                << RaDecCoord::DecToString(source.MeanDec()) << " "
                << std::setprecision(15)
                << source.TotalFlux(evaluateFrequency,
                                    aocommon::Polarization::StokesI)
                << '\n';
    }
  }
