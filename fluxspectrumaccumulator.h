...
class FluxSpectrumAccumulator : public Serializable {
 public:
   ...
   */
  FluxSpectrumAccumulator(const ModelComponent& component,
                          const BandData* bandData, size_t channelBlocks,
    ...
    long double l, m;
    ImageCoordinates::RaDecToLM<long double>(component.PosRA(),
                                             component.PosDec(), phaseCentreRA,
                                             phaseCentreDec, l, m);
    double stokesValues[4];
    std::complex<double> linearValues[4];
    for (size_t ch = 0; ch != bandData->ChannelCount(); ++ch) {
      for (size_t p = 0; p != 4; ++p)
        stokesValues[p] = component.SED().FluxAtFrequency(
            bandData->ChannelFrequency(ch), Polarization::IndexToStokes(p));
      Polarization::StokesToLinear(stokesValues, linearValues);
      _accumulators[ch] = new FluxAccumulator(
          l, m, bandData->ChannelWavelength(ch), linearValues);
    }
  }
  ...
  void UpdateBeam(LBeamEvaluator& evaluator, const double* ionG,
                  const double* ionDL, const double* ionDM) {
    LBeamEvaluator::PrecalcPosInfo posInfo;
    evaluator.PrecalculatePositionInfo(posInfo, _component.PosRA(),
                                       _component.PosDec());
    MC2x2 beamGains;
    for (size_t ch = 0; ch != _bandData->ChannelCount(); ++ch) {
      size_t channelBlockIndex =
          ch * _channelBlocks / _bandData->ChannelCount();
      const double frequency = _bandData->ChannelFrequency(ch);
      evaluator.Evaluate(posInfo, frequency, 0,
                         beamGains); /* TODO antenna index! */
      _accumulators[ch]->UpdateBeam(beamGains, ionG[channelBlockIndex],
                                    ionDL[channelBlockIndex],
                                    ionDM[channelBlockIndex]);
    }
  }

  void Accumulate(std::complex<double>* data, const double weight, size_t ch,
                  double u, double v, double w) {
    _accumulators[ch]->Add(data, weight, u, v, w);
  }

  void Finish() {
    for (std::vector<FluxAccumulator*>::iterator i = _accumulators.begin();
         i != _accumulators.end(); ++i)
      (*i)->Finish();
  }
  ...
  virtual void Unserialize(std::istream& stream) {
    ...
    if (size != _accumulators.size())
      throw std::runtime_error(
          "Unserializing flux spectrum accumulator with different number of "
          "channels: some of your input data do not match");
    for (size_t i = 0; i != size; ++i) _accumulators[i]->Unserialize(stream);
  }
  ...
  void SaveSpectrumAsTextFile(const char* filename) const {
    ...
    for (size_t ch = 0; ch != _bandData->ChannelCount(); ++ch) {
      ...
      for (size_t p = 0; p != 4; ++p) stream << '\t' << stokesValues[p];
    }
  }
