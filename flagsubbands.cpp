...
int main(int argc, char **argv) {
  if (argc < 4) {
    std::cout << "Usage: flagsubbands <ms> <subband count> <list of subbands>\n"
                 "The list of sub-bands are zero indexed and should be "
                 "separated by spaces.\n";
  } else {
    ...
    if (channelCount % subbandCount != 0)
      throw std::runtime_error(
          "Channel counts is not divisable by subband count");
     ...
     * Apply flags
     */
    std::cout << "Flagging " << subbands.size() << " subbands... "
              << std::flush;
