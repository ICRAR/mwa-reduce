...
IonPeeler::IonPeeler(size_t cpuCount)
    : _solutionInterval(1),
      _fitIterationCount(3),
      _applyBeam(true),
      _channelBlockSize(0),
      _channelBlockCount(1),
      _cpuCount(cpuCount),
      _weightMode(WeightMode::NaturalWeighted),
      _weightGridSize(0),
      _weightPixelScale(0.0),
      _clusterFluxLimit(0.0),
      _distanceLimit(0.0),
      _minUVSq(0.0),
      _verbose(false) {}
...
void IonPeeler::initWeighting(casacore::MeasurementSet& ms) {
  if (_weightGridSize > 0) {
    _imageWeights.reset(new ImageWeights(
        _weightMode, _weightGridSize, _weightGridSize, _weightPixelScale,
        _weightPixelScale, false, _weightPixelScale));
    if (_weightMode.RequiresGridding()) {
      std::cout << "Precalculating weights for " << _weightMode.ToString()
                << " weighting...\n";
...
void IonPeeler::Peel(const char* msName, const char* modelName,
  ...
  if (_dataColumnName.empty()) {
    ...
    if (hasCorrected) {
      std::cout << "Measurement set has corrected data: tasks will be applied "
                   "on the corrected data column.\n";
      _dataColumnName = "CORRECTED_DATA";
    } else {
      std::cout << "No corrected data in measurement set: tasks will be "
  ...
  long double phaseCentreRA = *phaseDirIter;
  ++phaseDirIter;
  long double phaseCentreDec = *phaseDirIter;
  // By setting the time beforehand, we don't waste time calculating a time step
  // we don't need.
  casacore::MEpoch::ROScalarColumn timeMColumn(
  ...
  for (size_t clusterIter = 0; clusterIter != clusterNames.size();
    ...
    _predicters[clusterIter] = new Predicter(
        phaseCentreRA, phaseCentreDec, _bandData.LowestFrequency(),
        _bandData.HighestFrequency(), channelCount);
    if (_applyBeam)
    ...
    double distance = aocommon::ImageCoordinates::AngularDistance<double>(
                          phaseCentreRA, phaseCentreDec, cluster.MeanRA(),
                          cluster.MeanDec()) *
                      (180.0 / M_PI);

    if (_distanceLimit > 0.0 && distance > _distanceLimit) {
      std::cout << "Cluster " << clusterNames[clusterIter] << " is " << distance
                << " deg away: removing cluster.\n";
      clustersToRemove.insert(clusterIter);
    } else if (_clusterFluxLimit > 0.0 &&
               _predicters[clusterIter]->TotalFlux(0) < _clusterFluxLimit) {
      std::cout << "Direction for " << clusterNames[clusterIter]
                << " is below cluster flux limit ("
                << _predicters[clusterIter]->TotalFlux(0) << " < "
                << _clusterFluxLimit << "): removing cluster.\n";
      clustersToRemove.insert(clusterIter);
    }
  }

  for (std::set<size_t>::const_reverse_iterator i = clustersToRemove.rbegin();
       i != clustersToRemove.rend(); ++i) {
    const size_t s = *i;
    _predictionModels.erase(_predictionModels.begin() + s);
    delete _predicters[s];
    _predicters.erase(_predicters.begin() + s);
  }
  std::cout << "Filtered model has " << _predictionModels.size()
            << " clusters.\n";
  ...
  double time = -1.0;
  aocommon::UVector<size_t> timestepRows;
  ...
  casacore::ROArrayColumn<float> weightColumn(
      ms, ms.columnName(casacore::MSMainEnums::WEIGHT_SPECTRUM));
  casacore::ROArrayColumn<bool> flagColumn(
      ms, ms.columnName(casacore::MSMainEnums::FLAG));
  casacore::ROScalarColumn<int> ant1Column(
      ms, ms.columnName(casacore::MSMainEnums::ANTENNA1));
  casacore::ROScalarColumn<int> ant2Column(
  ...
  _passCount =
      (_solutionInterval == 0)
          ? 1
          : (timestepCount + _solutionInterval - 1) / _solutionInterval;
  _channelBlockCount = _bandData.ChannelCount() / _channelBlockSize;
  std::cout << "Will process " << channelCount << " channels in "
            << _channelBlockCount << " channel groups\n";
  ...
  for (size_t clusterIter = 0; clusterIter != clusterNames.size();
    ...
    for (SourceGroup::const_iterator s = cluster.begin(); s != cluster.end();
         ++s)
  ...
  for (_pass = 0; _pass != _passCount; ++_pass) {
    ...
    if (beamEvaluator.Time().getValue() !=
        timeMColumn(_curStartRow).getValue()) {
    ...
    std::cout << "Reading (T " << _startTimestep << "-" << _endTimestep << "/"
              << timestepCount << ", rows " << _curStartRow << '-' << _curEndRow
              << '/' << ms.nrow() << ")...\n";
    for (size_t channelBlockIndex = 0; channelBlockIndex != _channelBlockCount;
         ++channelBlockIndex) {
      size_t channelIndexStart = channelBlockIndex * _bandData.ChannelCount() /
                                 _channelBlockCount,
             channelIndexEnd = (channelBlockIndex + 1) *
                               _bandData.ChannelCount() / _channelBlockCount,
             channelBlockSize = channelIndexEnd - channelIndexStart;
      _dataArrays[channelBlockIndex] =
          new VisibilityArray<std::complex<double>, 4>(
              1, _antennaCount, timestepsInPass * channelBlockSize);
      _weightArrays[channelBlockIndex] = new VisibilityArray<double, 1>(
          1, _antennaCount, timestepsInPass * channelBlockSize);
    }
    ...
    for (size_t rowIndex = _curStartRow; rowIndex != _curEndRow; ++rowIndex) {
      ...
      if (rowData.a1 != rowData.a2) {
        ...
        ++uvwI;
        rowData.w = *uvwI;
        double uvDistSq = rowData.u * rowData.u + rowData.v * rowData.v +
                          rowData.w * rowData.w;
        ...
        for (size_t cb = 0; cb != _channelBlockCount; ++cb) {
          size_t channelIndexStart =
                     cb * _bandData.ChannelCount() / _channelBlockCount,
                 channelIndexEnd =
                     (cb + 1) * _bandData.ChannelCount() / _channelBlockCount;
          for (size_t channelIndex = channelIndexStart;
               channelIndex != channelIndexEnd; ++channelIndex) {
            size_t timeIndexOffset = (channelIndex - channelIndexStart) *
                                     (_endTimestep - _startTimestep);
            for (size_t p = 0; p != 4; ++p) {
              if (flagPtr[p]) weightsReadPtr[p] = 0.0;
            }

            std::complex<double>* arrPtr = _dataArrays[cb]->ValuePtr(
                rowData.a1, rowData.a2,
                timeIndex - _startTimestep + timeIndexOffset);
            arrPtr[0] = dataPtr[0];
            arrPtr[1] = dataPtr[1];
            arrPtr[2] = dataPtr[2];
            arrPtr[3] = dataPtr[3];
            ...
            double imageWeight = (_imageWeights == 0)
                                     ? 1.0
                                     : _imageWeights->GetWeight(uInL, vInL);
            double* weightsWritePtr = _weightArrays[cb]->ValuePtr(
                rowData.a1, rowData.a2,
                timeIndex - _startTimestep + timeIndexOffset);
            if (uvDistSq < _minUVSq)
              weightsWritePtr[0] = 0.0;
            else {
              // GSL expects 1.0 / sigma -- these weights are 1.0 / variance, so
              // sqrt:
              weightsWritePtr[0] = sqrt(
                  (weightsReadPtr[0] + weightsReadPtr[3]) * 0.5 * imageWeight);
            }
    ...
    for (size_t i = 0; i != _cpuCount; ++i)
      threads.emplace_back(&IonPeeler::processingThreadFunction, this, &mutex,
                           &tasks);
    for (size_t i = 0; i != _cpuCount; ++i) threads[i].join();
      ...
      if (rowData.a1 != rowData.a2) {
        ...
        for (size_t cb = 0; cb != _channelBlockCount; ++cb) {
          ...
          for (size_t ch = channelIndexStart; ch != channelIndexEnd; ++ch) {
            size_t timeIndexOffset =
                (ch - channelIndexStart) * (_endTimestep - _startTimestep);
            std::complex<double>* arrPtr = _dataArrays[cb]->ValuePtr(
                rowData.a1, rowData.a2,
                timeIndex - _startTimestep + timeIndexOffset);
            dataPtr[0] = arrPtr[0];
    ...
    for (size_t cb = 0; cb != _channelBlockCount; ++cb) {
      if (_failedConvergencesPerChannelGroup[cb] != 0) {
        std::cout << "Warning: Solutions for channel group " << cb
                  << " failed to converge "
                  << _failedConvergencesPerChannelGroup[cb]
                  << "x within 100 iterations.\n";
      }
    ...
    for (size_t s = 0; s != _predictionModels.size(); ++s) {
      if (_failedConvergencesPerSource[s] != 0) {
        ...
        std::cout << "Warning: Solutions for " << firstSource.ClusterName()
                  << " failed to converge " << _failedConvergencesPerSource[s]
                  << "x within 100 iterations.\n";
      }
    }
  }

  for (size_t s = 0; s != _predictionModels.size(); ++s) delete _predicters[s];
}

void IonPeeler::processingThreadFunction(std::mutex* mutex,
                                         std::vector<size_t>* tasks) {
  PeelingStats stats;
  std::unique_lock<std::mutex> lock(*mutex);
  while (!tasks->empty()) {
    _progressBar->SetProgress(_channelBlockCount - tasks->size(),
                              _channelBlockCount);
...
int IonPeeler::posMinimizationFunc(const gsl_vector* xvec, void* data,
                                   gsl_vector* f) {
  const FittingInfo& fittingInfo = *reinterpret_cast<FittingInfo*>(data);
  const IonPeeler& ionPeeler =
      *reinterpret_cast<IonPeeler*>(fittingInfo.ionPeeler);
  ...
  for (size_t ch = fittingInfo.startChannel; ch != fittingInfo.endChannel;
       ++ch) {
    const double lambda = ionPeeler._bandData.ChannelWavelength(ch);
    size_t timeIndexOffset =
        (ch - fittingInfo.startChannel) *
        (ionPeeler._endTimestep - ionPeeler._startTimestep);
    for (size_t row = ionPeeler._curStartRow; row != ionPeeler._curEndRow;
         ++row) {
      const RowData& rowData = ionPeeler._rowData[row - ionPeeler._curStartRow];
      if (rowData.a1 != rowData.a2) {
        const std::complex<double>* dataPtr =
            ionPeeler._dataArrays[fittingInfo.channelBlockIndex]->ValuePtr(
                rowData.a1, rowData.a2,
                rowData.timeIndex - startTimestep + timeIndexOffset);
        // These weights are in 1.0/sigma (so have already been squarerooted
        // suitable for GSL)
        const double weight =
            *ionPeeler._weightArrays[fittingInfo.channelBlockIndex]->ValuePtr(
                rowData.a1, rowData.a2,
                rowData.timeIndex - startTimestep + timeIndexOffset);
        ...
        if (isfinite(dataPtr[0]) && isfinite(dataPtr[3])) {
          ...
          value = (g * (rotModelXX + rotModelYY) - dataPtr[0] - dataPtr[3]) *
                  0.5 * weight;
        }
...
int IonPeeler::posMinimizationFuncDeriv(const gsl_vector* xvec, void* data,
                                        gsl_matrix* J) {
  const FittingInfo& fittingInfo = *reinterpret_cast<FittingInfo*>(data);
  const IonPeeler& ionPeeler =
      *reinterpret_cast<IonPeeler*>(fittingInfo.ionPeeler);
...
int IonPeeler::posMinimizationFuncBoth(const gsl_vector* x, void* data,
...
void IonPeeler::positionFitter(size_t channelBlockIndex, PeelingStats& stats) {
  size_t channelIndexStart =
             channelBlockIndex * _bandData.ChannelCount() / _channelBlockCount,
         channelIndexEnd = (channelBlockIndex + 1) * _bandData.ChannelCount() /
                           _channelBlockCount,
  ...
  dataValueCount *=
      2 /* For real and imaginary */ * (channelIndexEnd - channelIndexStart);
  gsl_multifit_fdfsolver* solver =
      gsl_multifit_fdfsolver_alloc(T, dataValueCount, 3);

  aocommon::UVector<double> _solutionsG(_predictionModels.size()),
      _solutionsDL(_predictionModels.size()),
      _solutionsDM(_predictionModels.size());

  // The array containing for each source, the model data per cross-correlated
  // row: modelData[s][row*4+p]
  std::vector<aocommon::UVector<std::complex<double>>> modelDataPerSource(
      _predictionModels.size());

  for (size_t sourceIndex = 0; sourceIndex != _predictionModels.size();
       ++sourceIndex) {
    aocommon::UVector<std::complex<double>>& modelData =
        modelDataPerSource[sourceIndex];
  ...
  for (size_t sourceIndex = 0; sourceIndex != _predictionModels.size();
    ...
    aocommon::UVector<std::complex<double>>& modelData =
        modelDataPerSource[sourceIndex];
    aocommon::UVector<std::complex<double>>::iterator modelPtr =
        modelData.begin();
    for (size_t ch = channelIndexStart; ch != channelIndexEnd; ++ch) {
      const double lambda = _bandData.ChannelWavelength(ch);
      for (size_t row = _curStartRow; row != _curEndRow; ++row) {
        ...
        if (rowData.a1 != rowData.a2) {
          const double uOverL = rowData.u / lambda, vOverL = rowData.v / lambda,
                       wOverL = rowData.w / lambda;
          _predicters[sourceIndex]->Predict4(
              &*modelPtr, _predictionModels[sourceIndex], uOverL, vOverL,
              wOverL, ch, rowData.a1, rowData.a2);
          modelPtr += 4;
        }
      }
    }
  ...
  for (size_t fitIteration = 0; fitIteration != _fitIterationCount;
       ++fitIteration) {
    for (size_t sourceIndex = 0; sourceIndex != _predictionModels.size();
         ++sourceIndex) {
      aocommon::UVector<std::complex<double>>& modelData =
      ...
      double &g = _solutionsG[sourceIndex], &dl = _solutionsDL[sourceIndex],
      ...
      if (fitIteration != 0 && std::isfinite(g)) {
        ...
        for (size_t ch = channelIndexStart; ch != channelIndexEnd; ++ch) {
          ...
          for (size_t row = _curStartRow; row != _curEndRow; ++row) {
            const RowData& rowData = _rowData[row - _curStartRow];
            if (rowData.a1 != rowData.a2) {
              std::complex<double>* dataPtr =
                  _dataArrays[channelBlockIndex]->ValuePtr(
                      rowData.a1, rowData.a2,
                      rowData.timeIndex - _startTimestep + timeIndexOffset);
              double uOverL = rowData.u / lambda, vOverL = rowData.v / lambda;
              double expTerm = (uOverL * dl + vOverL * dm) * 2.0 * M_PI;
              double sExp, cExp;
              sincos(expTerm, &sExp, &cExp);
      ...
      do {
        ...
        if (status) {
          if (_verbose) {
            std::cout
                << "Warning: GSL fitter (gsl_multifit_fdfsolver_iterate()) "
                   "reported an error before converging! (niter="
        ...
        if (_verbose && status != GSL_SUCCESS && status != GSL_CONTINUE) {
          std::cout << "Warning: GSL fitter (gsl_multifit_test_delta()) "
                       "reported an error before converging! (niter="
                    << iter << ", status=" << gsl_strerror(status)
                    << ")\nSomething might be wrong with your data.\n";
        }
      ...
      if (sourceIndex <= 4 && _verbose)
        std::cout << "Solution " << fitIteration << ", channels "
                  << channelIndexStart << '-' << channelIndexEnd << " for "
                  << _predictionModels[sourceIndex].Source(0).ClusterName()
                  << ": " << g << ',' << dl << ',' << dm << '\n';
      // std::cout << gsl_strerror(status) << "\n";
      // std::cout << (status==GSL_CONTINUE ? "CONTINUE " : "X ") << iter << ",
      // g=" << g << ",dl=" << dl << ",dm=-" << dm << '\n';
      /*if(channelIndex == 5)
      {
  ...
  _solutionFile->WriteChannelBlock(solutions.data(), _pass, channelBlockIndex,
                                   0);
}
...
void IonPeeler::outputStats(const IonPeeler::PeelingStats& stats) {
  std::cout << "Avg lsiterations="
            << double(stats.lsFittingIterations) / stats.lsFits
            << ", avg g=" << stats.gSum / stats.lsFits
            << ", ion avg=" << radToString(stats.dlSum / stats.lsFits) << ','
            << radToString(stats.dmSum / stats.lsFits)
            << ", ion RMS=" << radToString(sqrt(stats.dlSumSq / stats.lsFits))
            << ',' << radToString(sqrt(stats.dmSumSq / stats.lsFits)) << '\n';
}
