...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cout
        << "Usage: apparently [-ms <ms>] [-beam <beamfitsfile>] [-plot] [-save "
           "<model>] <model> <fitsfile>\n"
    ...
    double *image =
        new double[fitsReader.ImageWidth() * fitsReader.ImageHeight()];
    fitsReader.Read(image);

    std::string setDescription;
    if (!msFilename.empty()) {
      casacore::MeasurementSet ms(msFilename);
      casacore::ROScalarColumn<double> timeCol(
    ...
    if (!beamFilename.empty()) {
      aocommon::FitsReader beamReader(beamFilename);
    ...
    for (Model::iterator s = model.begin(); s != model.end(); ++s) {
      ...
      aocommon::ImageCoordinates::RaDecToLM<long double>(
          source.Peak().PosRA(), source.Peak().PosDec(),
          fitsReader.PhaseCentreRA(), fitsReader.PhaseCentreDec(), l, m);
      int x, y;
      aocommon::ImageCoordinates::LMToXY<long double>(
          l, m, fitsReader.PixelSizeX(), fitsReader.PixelSizeY(), width, height,
          x, y);
    ...
    if (!outputFilename.empty()) measuredModel.Save(outputFilename);
  }
}
