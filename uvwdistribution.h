...
class UvwDistribution {
 private:
  typedef aocommon::UVector<std::pair<double, double>> WeightMap;
 ...
 public:
  ...
  void Calculate(casacore::MeasurementSet& ms) {
    ...
    for (size_t row = 0; row != ms.nrow(); ++row) {
      if (startTime != timeColumn(row)) break;
    ...
    aocommon::UVector<size_t> uvwHist(_binCount, 0),
        baselineLengthHist(_binCount, 0);
    for (size_t row = 0; row != ms.nrow(); ++row) {
      if (startTime != timeColumn(row)) break;
      ...
      if (!skip) {
        for (size_t ch = 0; ch != bandData.ChannelCount(); ++ch) {
          ...
          double bin =
              round((uvwDist - _minUVW) * double(_binCount - 1) / uvwRange);
      ...
      double bbin = round((distInM - _minBaseline) * double(_binCount - 1) /
                          baselineRange);
    ...
    for (size_t i = 0; i != uvwHist.size(); ++i) {
      ...
      double bbinCentre =
          (double(i) * baselineRange / double(_binCount) + _minBaseline);
      _baselineLengthMap[i] =
          std::make_pair(bbinCentre, baselineLengthHist[i] * 2);
    }
  ...
  double WeightWithInterpolation(double uvwDistance) const {
    const WeightMap::const_iterator upper = std::lower_bound(
        _weightMap.begin(), _weightMap.end(), std::make_pair(uvwDistance, 0.0));
    if (upper == _weightMap.end()) return _weightMap.back().second;
    if (upper == _weightMap.begin()) return _weightMap.front().second;
  ...
  double CumulativeCount(double baselineDist) const {
    double count = 0.0;
    const WeightMap::const_iterator upper =
        std::lower_bound(_baselineLengthMap.begin(), _baselineLengthMap.end(),
                         std::make_pair(baselineDist, 0.0));
    for (WeightMap::const_iterator i = upper; i != _baselineLengthMap.end();
         ++i) {
      count += i->second;
    }
    return count;
  ...
  void FitPowerlaw(double& exponent, double& factor) const {
    ...
    for (WeightMap::const_iterator i = _weightMap.begin();
         i != _weightMap.end(); ++i) {
 ...
 private:
  ...
  aocommon::UVector<std::pair<double, double>> _weightMap;
  aocommon::UVector<std::pair<double, double>> _baselineLengthMap;
