...
/**
 * The ImageBufferAllocator is a simple allocator for maintaining large memory
 * chunks.
 *
 * This class was made to avoid segmentation: it was found that wsclean
 * initially used more memory than it had allocated, which was due to
 * iteratively freeing and allocating large chunks, but because of intermediate
 * small allocations the large chunks no longer fitted, and new memory was
 * returned.
 */
class ImageBufferAllocator {
 public:
  ...
  class Ptr {
   public:
    ...
    Ptr() : _data(nullptr), _allocator(nullptr) {}
    Ptr(std::nullptr_t) : _data(nullptr), _allocator(nullptr) {}
    ...
    Ptr(Ptr&& source) : _data(source._data), _allocator(source._allocator) {
  ...
  ~ImageBufferAllocator() {
    ...
    for (std::vector<Buffer>::iterator i = _buffers.begin();
         i != _buffers.end(); ++i) {
      ...
      // In case of leak, leave this allocated so that e.g. valgrind can
      // diagnose it as well.
      if (!i->isFirstHalfUsed && !i->isSecondHalfUsed) free(i->ptr);
    }
    if (usedCount != 0) {
      std::cerr << usedCount
                << " image buffer(s) were still in use when image buffer "
                   "allocator was destroyed!\n"
                << str.str();
    }
  }
  ...
  void ReportStatistics() const {
    ...
    for (std::vector<Buffer>::const_iterator i = _buffers.begin();
         i != _buffers.end(); ++i) {
      totalSize += double(i->size) * double(sizeof(double) * 2);
    }
    Logger::Info << "Image buf alloc stats:\n"
                    "         max alloc'd images = "
                 << _nRealMax << " real + " << _nComplexMax
                 << " complex\n"
                    "       max allocated chunks = "
                 << _buffers.size()
                 << "\n"
                    "      current allocated mem = "
                 << round(totalSize / 1e8) / 10.0 << " GB \n";
  }

  void Allocate(size_t size, Ptr& ptr) { ptr.reset(Allocate(size), *this); }

  Ptr AllocatePtr(size_t size) { return Ptr(Allocate(size), *this); }
  ...
  void Free(double* buffer) {
    if (buffer != 0) {
      ...
      if (!found) {
        std::cerr << "Invalid or double call to "
                     "ImageBufferAllocator::Free(double*).\n";
        throw std::runtime_error(
            "Invalid or double call to ImageBufferAllocator::Free(double*).");
      }
      --_nReal;
    }
  }
  ...
  void Free(std::complex<double>* buffer) {
    if (buffer != 0) {
      ...
      if (!found) {
        std::cerr << "Invalid or double call to "
                     "ImageBufferAllocator::Free(std::complex<double>*).\n";
        throw std::runtime_error(
            "Invalid or double call to "
            "ImageBufferAllocator::Free(std::complex<double>*).");
      }
      --_nComplex;
    }
 ...
 private:
  ...
  Buffer* allocateNewBuffer(size_t size) {
    ...
    int errVal = posix_memalign(reinterpret_cast<void**>(&buffer->ptr),
                                sizeof(double) * 2, size * sizeof(double) * 2);
    if (errVal != 0) {
      std::ostringstream msg;
      msg << "posix_memalign() failed when allocating "
          << size * sizeof(double) * 2 << " bytes: ";
      switch (errVal) {
        case EINVAL:
          msg << "the alignment argument was not a power of two, or was not a "
                 "multiple of sizeof(void *)";
          break;
        case ENOMEM:
          msg << "there was insufficient memory to fulfill the allocation "
                 "request.";
          break;
        default:
...
class ImageBufferAllocator {
 public:
  ...
  double* Allocate(size_t size) { return new double[size]; }
  ...
  void Free(double* buffer) { delete[] buffer; }

  void Free(std::complex<double>* buffer) { delete[] buffer; }
