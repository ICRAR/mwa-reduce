...
class LogHistogram {
 ...
 public:
  ...
  double MaxAmplitude() const {
    if (_amplitudes.empty()) return 0.0;
  ...
  double NormalizedCount(double startAmplitude, double endAmplitude) const {
    ...
    for (std::map<double, class AmplitudeBin>::const_iterator i =
             _amplitudes.begin();
         i != _amplitudes.end(); ++i) {
  ...
  double NormalizedCount(double centreAmplitude) const {
    ...
    if (i == _amplitudes.end()) return 0.0;
    return (double)i->second.GetCount() /
           (binEnd(centreAmplitude) - binStart(centreAmplitude));
  }
  ...
  double MaxNormalizedCount() const {
    ...
    for (LogHistogram::const_iterator i = begin(); i != end(); ++i) {
      if (i.normalizedCount() > maxCount && i.value() > 0 &&
  ...
  double MinPosNormalizedCount() const {
    ...
    if (i == end()) return 0.0;
    double minCount =
        std::isfinite(i.normalizedCount()) ? i.normalizedCount() + 1.0 : 1.0;
    do {
      const double c = i.normalizedCount();
  ...
  double PowerLawExponentStdError(double startAmplitude,
                                  double expEstimator) const {
    long double termCount = 0.0;
    for (const_iterator i = begin(); i != end(); ++i) {
  ...
  double NormalizedSlopeOffset(double startAmplitude, double endAmplitude,
                               double slope) const {
    unsigned long n = 0;
  ...
  double NormalizedSlopeStdError(double startAmplitude, double endAmplitude,
                                 double slope) const {
    ...
    const long double avgX = xSum / (long double)n,
                      avgY = ySum / (long double)n;
    for (const_iterator i = begin(); i != end(); ++i) {
      if (i.value() >= startAmplitude && i.value() < endAmplitude) {
  ...
  double NormalizedSlopeStdDevBySampling(double startAmplitude,
                                         double endAmplitude, double slope,
  ...
  double PowerLawUpperLimit(double constrainingAmplitude, double exponent,
                            double factor) const {
    const double count = NormalizedCountAbove(constrainingAmplitude);
    const double term = count * (exponent + 1.0) / factor +
                        pow(constrainingAmplitude, exponent + 1.0);
    return pow(term, 1.0 / (exponent + 1.0));
  }

  double PowerLawLowerLimit(double constrainingAmplitude, double exponent,
                            double factor, double rfiRatio) const {
    const double countPart = NormalizedCountAbove(constrainingAmplitude);
    const double countTotal = NormalizedTotalCount() * rfiRatio;
    const double term = (countPart - countTotal) * (exponent + 1.0) / factor +
                        pow(constrainingAmplitude, exponent + 1.0);
    return pow(term, 1.0 / (exponent + 1.0));
  }

  double PowerLawLowerLimit2(double constrainingAmplitude, double exponent,
                             double factor, double rfiRatio) const {
    const double countPart = NormalizedCountAbove(constrainingAmplitude);
    const double countTotal = NormalizedTotalCount() * rfiRatio;
    const double term = (countPart - countTotal) * (exponent + 1.0) / factor +
  ...
  void Rescale(double factor) {
    ...
    for (std::map<double, AmplitudeBin>::const_iterator i = _amplitudes.begin();
         i != _amplitudes.end(); ++i) {
      position = newAmplitudes.insert(
          position, std::pair<double, AmplitudeBin>(
                        getCentralAmplitude(i->first * factor), i->second));
    }
  ...
  class const_iterator {
   public:
    ...
    double normalizedCount() const {
      return _iterator->second.GetCount() / (binEnd() - binStart());
    }
    long unsigned unnormalizedCount() const {
      return _iterator->second.GetCount();
    }
    double binStart() const {
      return _iterator->first > 0.0 ? exp10(log10(_iterator->first) - 0.005)
  ...
  const_iterator begin() const { return const_iterator(_amplitudes.begin()); }

  const_iterator end() const { return const_iterator(_amplitudes.end()); }
 ...
 private:
  ...
  AmplitudeBin &getBin(double centralAmplitude) {
    ...
    if (element == _amplitudes.end()) {
      element = _amplitudes
                    .insert(std::pair<double, AmplitudeBin>(centralAmplitude,
                                                            AmplitudeBin()))
                    .first;
    }
  ...
  double binStart(double x) const {
    return x > 0.0 ? exp10(log10(x) - 0.005) : -exp10(log10(x) - 0.005);
  }
