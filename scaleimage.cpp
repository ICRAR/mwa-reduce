...
int main(int argc, char* argv[]) {
  if (argc != 6)
    std::cout
        << "Syntax: scaleimage <cross-matched vo table> <model flux column "
           "name> <measured flux column name> <input fits> <output fits>\n";
  else {
    modelFluxCol = argv[2];
    ...
    aocommon::FitsReader reader(argv[4]);
    aocommon::UVector<double> values(reader.ImageWidth() *
                                     reader.ImageHeight());
    reader.Read(values.data());

    for (aocommon::UVector<double>::iterator i = values.begin();
         i != values.end(); ++i)
      *i *= factor;
