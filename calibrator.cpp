...
void Calibrator::Perform() {
  if (_verbose) std::cout << "Reading meta data... " << std::flush;
  ...
  aocommon::BandData bandData(_ms.spectralWindow());
  ...
  casacore::ROScalarColumn<double> timeColumn(
      _ms, _ms.columnName(casacore::MSMainEnums::TIME));
  casacore::ROArrayColumn<complex_t> dataColumn(_ms, _dataColumnName);
  casacore::ROArrayColumn<float> weightColumn(
      _ms, _ms.columnName(casacore::MSMainEnums::WEIGHT_SPECTRUM));
  casacore::ROArrayColumn<bool> flagColumn(
      _ms, _ms.columnName(casacore::MSMainEnums::FLAG));

  casacore::IPosition dataShape = dataColumn.shape(0);
  unsigned polarizationCount = dataShape[0];

  if (polarizationCount != 4) throw std::runtime_error("Pol count in MS != 4");

  if (channelCount % _solutionChannels != 0) {
    std::cout << "WARNING: You've requested to solve for " << _solutionChannels
              << " channels\n"
              << "per solution. However, the total number of channels ("
              << channelCount << ") is\n"
              << "not divisable by this. Therefore, some solutions will have "
  ...
  for (size_t rowIndex = 0; rowIndex != _ms.nrow(); ++rowIndex) {
    if (timeColumn(rowIndex) != time) {
      ...
      } else if (_selection.HasInterval() &&
                 timestepRows.size() == _selection.IntervalEnd()) {
        selectedRows.push_back(rowIndex);
      }
  ...
  size_t intervalCount =
      (_solutionInterval != 0)
          ? (selTimestepCount + _solutionInterval - 1) / _solutionInterval
          : 1;
  if (_verbose)
    std::cout << "DONE (" << timestepRows.size() << " timesteps, "
              << selTimestepCount << " selected, " << intervalCount
              << " intervals)\n";

  if (!_modelFilename.empty()) {
    if (_verbose) std::cout << "Reading model... " << std::flush;
  ...
  for (size_t intervalIndex = 0; intervalIndex != intervalCount;
    ...
    std::cout << " >>> INTERVAL " << (intervalIndex + 1) << " / "
              << intervalCount << " <<<\n";
    size_t intervalTimestepStart =
               (intervalIndex * selTimestepCount) / intervalCount,
           intervalTimestepEnd =
               ((intervalIndex + 1) * selTimestepCount) / intervalCount,
    ...
    double memPerChannelBlock =
        samplesPerChannel * 5 * sizeof(double) * _solutionChannels;
    if (_verbose) {
      std::cout << "Will use " << _threadCount << " cores.\n";
      std::cout << "Detected " << round(memSizeInGB * 10.0) / 10.0
                << " GB of system memory.\n";
      std::cout << "One channel block of " << _solutionChannels
                << " channels takes "
                << round(memPerChannelBlock * 10.0 / (1024 * 1024)) / 10.0
                << " MB of mem.\n";
    }
    size_t chBlocksPerPass = memSize / memPerChannelBlock;
    if (chBlocksPerPass > chBlockCount) chBlocksPerPass = chBlockCount;
    if (chBlocksPerPass == 0) {
      if (_verbose)
        std::cout << "WARNING: NOT ENOUGH MEMORY FOR EVEN ONE CHANNEL BLOCK, "
                     "expect very bad performance.\n";
      chBlocksPerPass = 1;
    }
    size_t passCount = (chBlockCount + chBlocksPerPass - 1) / chBlocksPerPass;
    if (_verbose)
      std::cout << "Number of channel that fit in memory: " << chBlocksPerPass
                << " blocks (" << chBlocksPerPass * _solutionChannels
                << " channels, " << passCount << " passes)\n";

    for (size_t pass = 0; pass != passCount; ++pass) {
      size_t startChBlock = (chBlockCount * pass) / passCount,
      ...
      for (size_t cb = 0; cb != partChBlockCount; ++cb) {
        const size_t cbChStart =
                         (cb + startChBlock) * channelCount / chBlockCount,
                     cbChEnd =
                         (cb + 1 + startChBlock) * channelCount / chBlockCount,
                     cbNCh = cbChEnd - cbChStart;
        std::unique_ptr<CalibrationMethod>& method = _calMethods[cb];
        method.reset(
            new CalibrationMethod(cbNCh, antennaCount, timestepsInInterval));
      ...
      predicter->SetChannelRange(startChBlock * channelCount / chBlockCount,
                                 endChBlock * channelCount / chBlockCount);
      ...
      while (predicter->GetNextRow(rowData)) {
        ...
        if (progress) {
          progress->SetProgress(rowData.rowIndex - intervalRowStart,
                                intervalRowEnd - intervalRowStart);
        }
        if (antenna1 != antenna2) {
          ...
          for (size_t cb = startChBlock; cb != endChBlock; ++cb) {
            ...
            _calMethods[cb - startChBlock]->AddData(
                &dataPtr[cbStartCh * 4], &weightsPtr[cbStartCh * 4],
                &modelValues[cbStartCh * 4], antenna1, antenna2,
                rowData.timeIndex);
          }
        }
      ...
      progress.reset();
      if (_verbose)
        std::cout << "Finished reading/predicting (" << selectedCount << "/"
                  << (selectedCount + notSelected)
                  << " rows selected).\nCalibrating...\n";
      ...
      for (size_t ant = 0; ant != antennaCount; ++ant) {
        for (size_t cb = 0; cb != partChBlockCount; ++cb) {
          ...
          aocommon::Matrix2x2::Invert(val);

          for (size_t p = 0; p != 4; ++p) {
            _solutionFile.WriteSolution(val[p], intervalIndex, ant,
                                        cb + startChBlock, p);
          }
        }
      }
      ...
      if (_savePlotFiles) {
        std::ofstream phasePlotStream(_phasePlotFilename.c_str()),
            gainPlotStream(_gainPlotFilename.c_str());
        ...
        for (size_t cb = 0; cb != partChBlockCount; ++cb) {
          ...
          for (size_t p = 0; p != 4; ++p) {
            for (size_t ant = 0; ant != antennaCount; ++ant) {
              ...
              double s1, s2;
              aocommon::Matrix2x2::SingularValues(val, s1, s2);
              switch (p) {
                case 0:
                  gainPlotStream << '\t' << s1;
                  break;
      ...
      if (_saveFaradayPlotFiles) {
        ...
        for (size_t cb = 0; cb != partChBlockCount; ++cb) {
          ...
          for (size_t ant = 0; ant != antennaCount; ++ant) {
            ...
            faradayPlotStream << '\t'
                              << -aocommon::Matrix2x2::RotationAngle(val);
          }
          faradayPlotStream << '\n';
        }
      }
        ...
        for (size_t cb = 0; cb != partChBlockCount; ++cb) {
          ...
          for (size_t ant = 0; ant != antennaCount; ++ant) {
            ...
            for (size_t p = 0; p != 4; ++p)
              val[p] = _calMethods[cb]->JonesSolution(ant, p);
            aocommon::Matrix2x2::Invert(val);
            double totalPower = std::abs(val[0]) + std::abs(val[1]) +
                                std::abs(val[2]) + std::abs(val[3]);
            crossTermPlotStream
                << '\t'
                << (std::abs(val[1]) + std::abs(val[2])) * 100.0 / totalPower;
          }
          crossTermPlotStream << '\n';
        }
      }
...
void Calibrator::calibrateChannelBlock(size_t channelBlockIndex,
                                       size_t threadIndex) {
  ...
  if ((iters >= _nIter || !std::isfinite(limit)) &&
      !method.OnlySolveRotation()) {
    std::cout << "Recalculating channel " << channelBlockIndex
              << " (accuracy=" << limit << ").\n";
    ...
    if ((iters >= _nIter && limit > _minAccuracy) || !std::isfinite(limit)) {
      std::cout << "Chanblock " << channelBlockIndex
                << " did not converge (accuracy=" << limit
                << "), setting gains to NaN.\n";
      method.InitSolutionsToNaN();
    } else {
      if (iters >= _nIter && limit > _stoppingAccuracy) {
        std::cout << "Chanblock " << channelBlockIndex
                  << " converged (accuracy=" << limit
                  << ") but did not reach stopping accuracy.\n";
      }
  ...
  if (channelBlockIndex <= 16) {
    if (_verbose)
      std::cout << "Current value of Jones matrix for ant " << _followAntenna
                << ", cb " << channelBlockIndex << ":\n"
                << CalibrationMethod::MatrixToString(
                       &method.JonesSolution(_followAntenna, 0));
  }

  if (_verbose)
    std::cout << "Finished calibrating chanblock " << channelBlockIndex
              << " in " << iters << " iterations, precision=" << limit << ".\n";
}
