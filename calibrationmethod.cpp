...
void CalibrationMethod::InitSolutionsToUnity() {
  ...
  for (size_t i = 0; i != _nAntenna; ++i) {
    *_jonesPtr = std::complex<double>(1.0, 0.0);
    ++_jonesPtr;
    *_jonesPtr = std::complex<double>(0.0, 0.0);
    ++_jonesPtr;
    *_jonesPtr = std::complex<double>(0.0, 0.0);
    ++_jonesPtr;
    *_jonesPtr = std::complex<double>(1.0, 0.0);
...
void CalibrationMethod::InitSolutionsToNaN() {
  const std::complex<double> nan(std::numeric_limits<double>::quiet_NaN(),
...
void CalibrationMethod::InitSolutions(const CalibrationMethod &source) {
  ...
  aocommon::UVector<std::complex<double>>::iterator ptr =
      _jonesSolutions.begin();
  for (size_t i = 0; i != _nAntenna; ++i) {
...
void CalibrationMethod::AddData(const std::complex<DataFloatType> *data,
                                const float *weights,
                                const std::complex<double> *predictedValues,
                                size_t antenna1, size_t antenna2,
                                size_t timestep) {
  std::complex<double> *destDataPtr =
      _data.ValuePtr(antenna1, antenna2, timestep);
  std::complex<double> *destModelPtr =
      _model.ValuePtr(antenna1, antenna2, timestep);
    ...
    for (size_t p = 0; p != 4; ++p) {
      if (std::isfinite(data->real()) && std::isfinite(data->imag())) {
        ...
        *destModelPtr = *predictedValues;
        if (*weights < minWeight) minWeight = *weights;
      } else {
...
template void CalibrationMethod::AddData(
    const std::complex<float> *data, const float *weights,
    const std::complex<double> *predictedValues, size_t antenna1,
    size_t antenna2, size_t timestep);

template void CalibrationMethod::AddData(
    const std::complex<double> *data, const float *weights,
    const std::complex<double> *predictedValues, size_t antenna1,
    size_t antenna2, size_t timestep);
...
double CalibrationMethod::totalDistance(size_t antenna) {
    ...
    for (size_t antenna1 = 0; antenna1 != _nAntenna; ++antenna1) {
      if (antenna1 != antenna) {
        ...
        const std::complex<double> *data =
            _data.ValuePtr(antenna, antenna1, timestep);
        const std::complex<double> *model =
            _model.ValuePtr(antenna, antenna1, timestep);
        ...
        for (size_t ch = 0; ch != _nChannels; ++ch) {
          if (isConjTranspose) {
            j1TimesD[0] =
                jones1[0] * std::conj(data[0]) + jones1[1] * std::conj(data[1]);
            j1TimesD[1] =
                jones1[0] * std::conj(data[2]) + jones1[1] * std::conj(data[3]);
            j1TimesD[2] =
                jones1[2] * std::conj(data[0]) + jones1[3] * std::conj(data[1]);
            j1TimesD[3] =
                jones1[2] * std::conj(data[2]) + jones1[3] * std::conj(data[3]);

            j1DJ2[0] = j1TimesD[0] * std::conj(jones2[0 /* (0^H) */]) +
                       j1TimesD[1] * std::conj(jones2[1 /* (2^H) */]);
            j1DJ2[1] = j1TimesD[0] * std::conj(jones2[2 /* (1^H) */]) +
                       j1TimesD[1] * std::conj(jones2[3 /* (3^H) */]);
            j1DJ2[2] = j1TimesD[2] * std::conj(jones2[0 /* (0^H) */]) +
                       j1TimesD[3] * std::conj(jones2[1 /* (2^H) */]);
            j1DJ2[3] = j1TimesD[2] * std::conj(jones2[2 /* (1^H) */]) +
                       j1TimesD[3] * std::conj(jones2[3 /* (3^H) */]);
          ...
          distance += std::norm(distances[0]) + std::norm(distances[1]) +
  ...
  std::cout << "XX flux for ant " << antenna << ": "
            << (xxFlux / double(xxFluxCount))
            << " model=" << (modelXX / double(xxFluxCount)) << '\n';
  if (weightSum == 0.0)
    return 0.0;
  else
...
void CalibrationMethod::Execute(double &precisionLimit, size_t &nIter) {
  ...
  aocommon::UVector<bool> antennaResults(_nAntenna, false);
  do {
    ++iterationNumber;
    ...
    aocommon::UVector<double> changeSizes(_nAntenna * 4, 0);
    ...
    for (size_t p = 0; p != 4; ++p) globalChangeSizes[p] = 0.0;
    for (size_t ant = 0; ant != _nAntenna; ++ant) {
      for (size_t p = 0; p != 4; ++p) {
    ...
    for (size_t p = 0; p != 4; ++p) {
      ...
      if (globalChangeSizes[p] > precisionLimit) continueIterating = true;
    }
  ...
  precisionLimit =
      std::max(std::max(globalChangeSizes[0], globalChangeSizes[1]),
...
std::string CalibrationMethod::MatrixToString(
    const std::complex<double> *matrix) {
  std::stringstream s;
  double s1, s2;
  Matrix2x2::SingularValues(matrix, s1, s2);
  s << " (" << matrix[0] << " " << matrix[1] << ")\tamplitudes=("
    << abs(matrix[0]) << ' ' << abs(matrix[1]) << ") SV=\t" << s1
    << "\n"
       " ("
    << matrix[2] << " " << matrix[3] << ")\t           (" << abs(matrix[2])
    << ' ' << abs(matrix[3]) << ")    \t" << s2 << '\n';
  return s.str();
}

void CalibrationMethod::calculateNextIter(size_t ant,
                                          std::complex<double> *nextJones,
                                          bool *antennaResults) {
  ...
  //            ( SUM_{k!=ant in Nant} DATA[ant,k] JONES^H[k] JONES[k]
  //            DATA^H[ant,k] )^-1
  ...
  if (Matrix2x2::MultiplyWithInverse(solutions, rTerm)) {
    ...
    for (size_t p = 0; p != 4; ++p) nextJones[p] = solutions[p];
  } else
    antennaResults[0] = false;
}

void CalibrationMethod::SolutionSingularValue(size_t antenna, double &s1,
                                              double &s2) const {
