...
class TriangleInterpolator {
 ...
 public:
  void Interpolate(double* grid, size_t gridWidth, size_t gridHeight, double x1,
                   double y1, double v1, double x2, double y2, double v2,
    ...
    if (minY >= gridHeight || maxY < 0.0) return;
    size_t minYi = size_t(std::max(minY, 0.0)),
    ...
    for (size_t yi = minYi; yi != maxYi; ++yi) {
      ...
      size_t rightX =
          size_t(ceil(std::max(std::min(leftX2, double(gridWidth)), 0.0)));
  ...
  void InterpolateConvexHullVertex(double* grid, size_t gridWidth,
                                   size_t gridHeight, double x1, double y1,
                                   double x2, double y2, double v2, double x3,
                                   double y3) const {
    if ((x1 == x2 && y1 == y2) || (x2 == x3 && y2 == y3)) return;
    double yd1 = x1 - x2, xd1 = y2 - y1, yd3 = x2 - x3, xd3 = y3 - y2;
    double yStart, yEnd;
    if (xd1 < 0.0)
  ...
  void InterpolateConvexHullEdge(double* grid, size_t gridWidth,
                                 size_t gridHeight, double x1, double y1,
                                 double v1, double x2, double y2,
                                 double v2) const {
    ...
    for (size_t yi = yiStart; yi < yiEnd; ++yi) {
      ...
      if (cx2 < 0.0)
        cx2 = 0.0;
      else if (cx2 > gridWidth)
        cx2 = gridWidth;
      double cm = GetXForYCrossing(y, x1, y1, x2, y2);
      if (cm < 0.0)
        cm = 0.0;
      else if (cm > gridWidth)
        cm = gridWidth;
      if (dx >= 0.0 && cm > cx1)
        cx1 = cm;
 ...
 private:
  static double PointToLineDistance(double px, double py, double lx1,
                                    double ly1, double lx2, double ly2) {
    double dx = lx2 - lx1, negDy = ly1 - ly2;
    return fabs(negDy * px + dx * py + (lx1 * ly2 - lx2 * ly1)) /
           sqrt(dx * dx + negDy * negDy);
  }

  static double GetXForYCrossing(double y, double lx1, double ly1, double lx2,
                                 double ly2) {
