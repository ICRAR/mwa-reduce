...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::cout << "Usage: flagmwa <ms> [<timestepcount> [<sbcount> "
                 "<centre-channels> <side-channels>]]\n"
                 "Flags the start timestep and corrupted centre channels of "
                 "each sub-band as needed for\n"
                 "MWA. timestepcount defaults to 5, sbcount defaults to 24, "
                 "centre and side channels to 1.\n";
  } else {
    MeasurementSet ms(argv[1], Table::Update);
    size_t timestepCount = 5, sbCount = 24, centreChannelCount = 1,
           sideChannelCount = 1;
    if (argc > 2) {
      ...
      if (argc > 3) {
        ...
        if (argc > 4) {
          ...
          if (argc > 5) sideChannelCount = atoi(argv[5]);
        }
      }
    }
    ...
    if (channelsPerSubband * sbCount != channelCount)
      throw std::runtime_error(
          "channels per subband != channelCount * subband count");
        ...
        if (timestepIndex < timestepCount) {
          ...
          for (size_t sb = 0; sb != sbCount; ++sb) {
            for (size_t ch = 0; ch != channelsPerSubband; ++ch) {
              bool doFlag = false;
              if (std::abs((long)(ch * 2 - channelsPerSubband)) * 2 <
                  (long)centreChannelCount)
                doFlag = true;
              if (ch < sideChannelCount ||
                  ch >= channelsPerSubband - sideChannelCount)
                doFlag = true;
              for (size_t p = 0; p != polarizationCount; ++p) {
                if (doFlag) {
                  *flagPtr = true;
    ...
    std::cout << " DONE (Set "
              << (round(flaggedCount * 1000.0 / totalCount) / 10.0)
              << "% flagged)\n";
  }
