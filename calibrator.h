...
class Calibrator {
 public:
  ...
  void SetMaxUVW(double maxUVW) { _maxUVW = maxUVW; }

  void SetSolutionInterval(size_t solutionInterval) {
    _solutionInterval = solutionInterval;
  }

  void SetSolutionChannels(size_t nChannels) { _solutionChannels = nChannels; }

  void SetVerbose(bool verbose) { _verbose = verbose; }

  void SetSavePlotFiles(bool savePlotFiles) { _savePlotFiles = savePlotFiles; }

  void SetPlotFilenames(const std::string& phasePlotFilename,
                        const std::string& gainPlotFilename) {
    _phasePlotFilename = phasePlotFilename;
    _gainPlotFilename = gainPlotFilename;
  }

  void SetAbsMem(double absmem) { _absmem = absmem; }
  ...
  SolutionFile& GetSolutionFile() { return _solutionFile; }
 ...
 private:
  ...
  std::string _phasePlotFilename, _gainPlotFilename, _faradayPlotFilename,
      _crossTermsPlotFilename, _mwaPath;
