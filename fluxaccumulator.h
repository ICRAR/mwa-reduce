...
class FluxAccumulator : public Serializable {
 public:
  ...
  FluxAccumulator(double l, double m, double lambda,
                  const std::complex<double>* modelFlux);
  ...
  void UpdateBeam(const MC2x2& beamGains, double ionG, double ionDL,
                  double ionDM);
  void Add(const std::complex<double>* vis, const double visWeight, double u,
           double v, double w);

  double L() const { return _l; }
  double M() const { return _m; }

  void Finish() { accumulateBeforeBeamChange(); }

  virtual void Serialize(std::ostream& stream) const {
    if (_accVisWeightBeforeBeamChange != 0.0)
      throw std::runtime_error(
          "Serializing flux accumulator that has not been finished!");
  ...
  virtual void Unserialize(std::istream& stream) {
    ...
    for (size_t p = 0; p != 4; ++p) _accFluxesBeforeBeamChange[p] = 0.0;
    _beamGains[0] = 1.0;
    _beamGains[1] = 0.0;
    _beamGains[2] = 0.0;
    _beamGains[3] = 1.0;
  ...
  void GetWeights(double* stokesMatrix) const {
    ...
    bool isValid = true;
    for (size_t p = 0; p != 4; ++p)
      if (!std::isfinite(stokesMatrix[p]) || stokesMatrix[p] == 0.0)
        isValid = false;
    if (!isValid) {
      for (size_t p = 0; p != 4; ++p)
        stokesMatrix[p] = std::numeric_limits<double>::quiet_NaN();
 ...
 private:
  static void serializeMatrix(std::ostream& stream,
                              const std::complex<double>* m) {
    for (size_t p = 0; p != 4; ++p) SerializeToDoubleC(stream, m[p]);
  }
  static void unserializeMatrix(std::istream& stream, std::complex<double>* m) {
    for (size_t p = 0; p != 4; ++p) m[p] = UnserializeDoubleC(stream);
  }
  static void unserializeAndAddMatrix(std::istream& stream,
  ...
  void accumulateBeforeBeamChange() {
    if (std::isfinite(_ionG)) {
      ...
      Matrix2x2::ScalarMultiply(
          temp2, _accVisWeightBeforeBeamChange * _ionG * (_ionG * _ionG));
      ...
      Matrix2x2::MultiplyAdd(_accWeights, temp2,
                             _accVisWeightBeforeBeamChange * (_ionG * _ionG));
    }
