...
class SourceMatcher {
 public:
  ...
  SourceMatcher()
      : _acceptAmbigiousDistanceFactor(10), _keepBaseSources(false) {}
  ...
  void Match(MatchingType matchingType, double distanceInRad, double weight,
             Model& baseModel, Model& addedModel, Model& outputModel) {
    baseModel.Sort();
    addedModel.Sort();
    ...
    if (matchingType == FindNewMatching) {
      ...
      matchCombine(matchingType, distanceInRad, weight, baseModel, addedModel,
                   outputModel);
    }
  }

 private:
  void matchNew(double distanceInRad, Model& baseModel, Model& addedModel,
                Model& outputModel) {
    ...
    for (size_t i = 0; i != addedModel.SourceCount(); ++i) {
      ...
      for (ModelSource::const_iterator compIter = addSource.begin();
           compIter != addSource.end(); ++compIter) {
        for (size_t j = 0; j != baseModel.SourceCount(); ++j) {
          const ModelSource& baseSource = baseModel.Source(j);

          for (ModelSource::const_iterator baseIter = baseSource.begin();
               baseIter != baseSource.end(); ++baseIter) {
            double sourceDist = aocommon::ImageCoordinates::AngularDistance(
                compIter->PosRA(), compIter->PosDec(), baseIter->PosRA(),
                baseIter->PosDec());
    ...
    std::cout << "Unmatched: " << (addedModel.SourceCount() - matchedCount)
              << " Catalogues: " << baseModel.SourceCount() << " & "
              << addedModel.SourceCount() << '\n';
  }

  void matchCombine(MatchingType matchingType, double distanceInRad,
                    double weight, Model& baseModel, Model& addedModel,
                    Model& outputModel) {
    ...
    for (size_t i = 0; i != addedModel.SourceCount(); ++i) {
      ...
      for (ModelSource::const_iterator compIter = addSource.begin();
        ...
        for (size_t j = 0; j != baseModel.SourceCount(); ++j) {
          const ModelSource& baseSource = baseModel.Source(j);
          SourceInfo& baseInfo =
              *static_cast<SourceInfo*>(baseSource.UserData());
      ...
      if (matchingBaseSources.size() > 1) {
        ...
        for (size_t i = 0; i != matchingBaseSources.size(); ++i) {
          ...
          double sourceDist = aocommon::ImageCoordinates::AngularDistance(
              s.MeanRA(), s.MeanDec(), addSource.MeanRA(), addSource.MeanDec());
          distances.push_back(std::make_pair(sourceDist, &s));
          warningsBuffer << " - At "
                         << aocommon::units::Angle::ToNiceString(sourceDist)
                         << " : " << s.Name() << '\n';
        }
        std::sort(distances.begin(), distances.end());
        if (distances[0].first * _acceptAmbigiousDistanceFactor <=
            distances[1].first) {
          warningsBuffer << "Nearest is " << _acceptAmbigiousDistanceFactor
                         << "x closer, accepting "
                         << distances[0].second->Name() << " as match.\n";
      ...
      if (matchingBaseSources.size() == 1) {
        ...
        if (newSource.ComponentCount() != 1 || addSource.ComponentCount() != 1)
          ...
          if (matchingType == AddSpectraMatching) {
            newSource.front().MSED().CombineMeasurements(
                addSource.front().MSED());
            outputModel.AddSource(newSource);
          } else {
            newSource.front().MSED().CombineMeasurementsWithAveraging(
                addSource.front().MSED(), weight);
            outputModel.AddSource(newSource);
            if (matchingType == CompareMatching) {
              long double freq =
                  matchingBaseSource->GetIntegratedMSED().CentreFrequency();
              long double s1 = matchingBaseSource->GetIntegratedMSED()
                                   .FluxAtFrequencyFromIndex(freq, 0);
              long double s2 =
                  addSource.GetIntegratedMSED().FluxAtFrequencyFromIndex(freq,
                                                                         0);
              (*csvFile) << newSource.MeanRA() * 180.0 / M_PI << ','
                         << newSource.MeanDec() * 180.0 / M_PI << ',' << s1
                         << ',' << s2 << ',' << (s1 / s2) << '\n';
            }
          }
        }
    ...
    if (_keepBaseSources) {
      for (Model::const_iterator s = baseModel.begin(); s != baseModel.end();
           ++s) {
