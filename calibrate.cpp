...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    std::cout
        << "Usage: calibrate [-absmem <mem in GB>] [-p <phases.txt> "
           "<gains.txt>] [-pf <faraday.txt>] [-px <crossterms.txt>] [-minuv "
           "<min uvw dist in m>] [-maxuv <max uvw dist in m>] [-a "
           "<min-accuracy> <stop-accuracy>] [-i <niter>] [-j <threads>] [-m "
           "<model>] [-scalar] [-diag] [-rotation] [-applybeam] [-t timesteps] "
           "[-interval <starttime> <endtime>] [-ch channels per solution] "
           "[-datacolumn <name>] [-quiet] [-mwa-path path] <measurementset.ms> "
           "<solutions.bin>\n\n"
           "This will calculate \"static\" phase offsets for all stations. It "
           "produces approximate least-squares solutions.\n\n"
           "The official name of this algorithm is the \"Mitchcal\" algorithm. "
           "The following is a suggestion for referencing this algorithm in "
           "scientific articles:\n"
           "\" Calibration was performed with the full-Jones Mitchcal "
           "algorithm developed for MWA calibration, as described by Offringa "
           "et al. (2016) \"\n"
           "bibtex:\n"
           "@article{offringa-2016,\n"
           "  author = {Offringa, A. R. and Trott, C. M. and Hurley-Walker and "
           "others},\n"
           "  title = {Parametrizing Epoch of Reionization foregrounds: a deep "
           "survey of low-frequency point-source spectra with the Murchison "
           "Widefield Array},\n"
           "  volume = {458}, \n"
           "  number = {1}, \n"
           "  pages = {1057-1070}, \n"
           ...
           "  URL = "
           "{http://mnras.oxfordjournals.org/content/458/1/1057.abstract}, \n"
           "  journal = {MNRAS}\n}\n";
  } else {
    int argi = 1;
    // bool saveCrossTermsPlotFile = false, saveFaradayPlotFiles = false;
    bool savePlotFiles = false, applyBeam = false, onlyScalar = false,
         onlyDiag = false, onlyRotation = false, doQuiet = false,
         hasInterval = false;
    std::string plotPhaseFile, plotGainFile, plotFaradayFile,
        crossTermsPlotFile, modelFile, mwaPath;
    ...
    double minAccuracy = CalibrationMethod::DefaultMinAccuracy(),
           ...
           minUVW = 0.0, maxUVW = 10000000.0, absmem = 0.0;
    ...
    while (argv[argi][0] == '-') {
      ...
      if (param == "p") {
        ...
        throw std::runtime_error(std::string("Invalid parameter ") +
                                 argv[argi]);
      ++argi;
    }
    ...
    if (hasInterval) calibrator.SetInterval(intervalStart, intervalEnd);
    if (savePlotFiles) {
