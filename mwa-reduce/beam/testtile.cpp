...
void plotArrayJones(size_t npix, const std::vector<std::complex<double>>& js, double za, double freq)
{
	...
	for(size_t i=0; i!=size; ++i)
		image[i] = std::abs(js[i*4 + 0]);
	std::ostringstream s1;
	s1 << "/tmp/MWA_J00_voltage_mag_" << (freq/1e6) << "MHz_ZA" << za << ".fits";
	writer.Write(s1.str(), image.data());

	for(size_t i=0; i!=size; ++i)
		image[i] = std::abs(js[i*4 + 1]);
	std::ostringstream s2;
	s2 << "/tmp/MWA_J01_voltage_mag_" << (freq/1e6) << "MHz_ZA" << za << ".fits";
	writer.Write(s2.str(), image.data());

	for(size_t i=0; i!=size; ++i)
		image[i] = std::abs(js[i*4 + 2]);
	std::ostringstream s3;
	s3 << "/tmp/MWA_J10_voltage_mag_" << (freq/1e6) << "MHz_ZA" << za << ".fits";
	writer.Write(s3.str(), image.data());

	for(size_t i=0; i!=size; ++i)
		image[i] = std::abs(js[i*4 + 3]);
	std::ostringstream s4;
	s4 << "/tmp/MWA_J11_voltage_mag_" << (freq/1e6) << "MHz_ZA" << za << ".fits";
...
int main(int argc, char* argv[])
{
	...
	for(size_t i=0; i!=3; ++i)
	{
		...
		plotArrayJones(npix, js, zaOfDelays[i], freq);
