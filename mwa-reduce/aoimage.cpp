...
int main(int argc, char *argv[])
{
	...
	while(argi < argc)
	{
		...
		if(modelFilename != 0)
		{
			...
			renderer.Restore(imageData, imager.ImageSize(), imager.ImageSize(), model, 0.5 / imager.OverallMaxUVDist(), imageInfo.lowestFrequency, imageInfo.highestFrequency, Polarization::StokesI);
			std::cout << "DONE\n";
		}
