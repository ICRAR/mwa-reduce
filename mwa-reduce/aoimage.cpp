...
int main(int argc, char *argv[])
{
	...
	while(argi < argc)
	{
		...
		if(modelFilename != 0)
		{
			...
			renderer.Render(imageData, imager.ImageSize(), model, 0.5 / imager.OverallMaxUVDist(), imageInfo.lowestFrequency, imageInfo.highestFrequency);
			std::cout << "DONE\n";
		}
		...
		writer.Write(imageData, imager.ImageSize(), imageInfo.phaseCentreRA, imageInfo.phaseCentreDec, pixelScale, pixelScale);
		delete[] imageData;
		std::cout << "DONE\n";
	}
}
