...
int main(int argc, char *argv[])
{
	...
	double frequency = 0.0, bandwidth = 0.0;
	...
	imgWriter.Write<double>(outImage, width, height, ra, dec, pixelSizeX, pixelSizeY, frequency, bandwidth);
	delete[] outImage;
	
	FitsWriter weightsWriter(outWeightName);
	weightsWriter.Write<double>(outWeights, width, height, ra, dec, pixelSizeX, pixelSizeY, frequency, bandwidth);
	delete[] outWeights;
}
