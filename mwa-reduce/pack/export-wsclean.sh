...
if [[ $1 == "" ]] ; then
		...
		DEST="$1"
		cd ..
		cp -v angle.* areaset.* banddata.* buffered_lane.* cachedimageset.* fftconvolver.* fftresampler.* fitsiochecker.* fitsreader.* fitswriter.* gaussianfitter.* imagecoordinates.* imagebufferallocator.* imageweights.* inversionalgorithm.* lane.* layeredimager.* multibanddata.* nlplfitter.* matrix2x2.* model.* modelrenderer.* modelsource.* msselection.* polarizationenum.* progressbar.* radeccoord.* sourcesdf.* sourcesdfwithsamples.* spectralenergydistribution.* stopwatch.* uvector.* uvwdistribution.* weightmode.* wsclean.* wscleanmain.cpp wsinversion.* wscversion.h ${DEST}/
...
#		cp -v beam/lnaimpedance.* beam/tilebeam*.{h,cpp} beam/tileimpedance.{h,cpp} ${DEST}/beam/
		...
		cp -v parser/{areaparser.h,modelparser.h,tokenizer.h} ${DEST}/parser/		
		cat wscversion.h
fi
