...
struct BaselineData {
  BaselineData(size_t timestepCount, size_t channelCount)
  ...
  complex_t &DataX(size_t timeIndex, size_t freqIndex) {
    return _dataX[timeIndex * _channelCount + freqIndex];
  }
  complex_t &DataY(size_t timeIndex, size_t freqIndex) {
    return _dataY[timeIndex * _channelCount + freqIndex];
  }
  const complex_t &DataX(size_t timeIndex, size_t freqIndex) const {
    return _dataX[timeIndex * _channelCount + freqIndex];
  }
  const complex_t &DataY(size_t timeIndex, size_t freqIndex) const {
    return _dataY[timeIndex * _channelCount + freqIndex];
  }
...
struct BaselineWeights {
  BaselineWeights(size_t timestepCount, size_t channelCount)
      : _channelCount(channelCount) {
  ...
  float &WeightX(size_t timeIndex, size_t freqIndex) {
    return _weightX[timeIndex * _channelCount + freqIndex];
  }
  float &WeightY(size_t timeIndex, size_t freqIndex) {
    return _weightY[timeIndex * _channelCount + freqIndex];
  }
  const float &WeightX(size_t timeIndex, size_t freqIndex) const {
    return _weightX[timeIndex * _channelCount + freqIndex];
  }
  const float &WeightY(size_t timeIndex, size_t freqIndex) const {
    return _weightY[timeIndex * _channelCount + freqIndex];
  }
...
struct Data {
 ...
 public:
  Data(size_t antennaCount, size_t timestepCount, size_t channelCount)
    ...
    for (size_t a1 = 0; a1 != antennaCount; ++a1) {
      for (size_t a2 = a1 + 1; a2 != antennaCount; ++a2) {
        _predicted[a1 * _antennaCount + a2] =
            new BaselineData(timestepCount, channelCount);
        _measured[a1 * _antennaCount + a2] =
            new BaselineData(timestepCount, channelCount);
        _weights[a1 * _antennaCount + a2] =
            new BaselineWeights(timestepCount, channelCount);
      }
    }
  }
  ~Data() {
  ...
  BaselineData &Predicted(size_t a1, size_t a2) {
    return *_predicted[a1 * _antennaCount + a2];
  }
  BaselineData &Measured(size_t a1, size_t a2) {
    return *_measured[a1 * _antennaCount + a2];
  }
  BaselineWeights &Weights(size_t a1, size_t a2) {
    return *_weights[a1 * _antennaCount + a2];
  }
  const BaselineData &Predicted(size_t a1, size_t a2) const {
    return *_predicted[a1 * _antennaCount + a2];
  }
  const BaselineData &Measured(size_t a1, size_t a2) const {
    return *_measured[a1 * _antennaCount + a2];
  }
  const BaselineWeights &Weights(size_t a1, size_t a2) const {
    return *_weights[a1 * _antennaCount + a2];
  }
};
...
std::string PhaseToString(lcomplex_t val) {
  std::stringstream s;
  s << PhaseToString(atan2l(val.imag(), val.real())) << '{' << val.real() << ','
    << val.imag() << '}';
  return s.str();
}
...
lcomplex_t rotate(complex_t data, long double rotSin, long double rotCos) {
  return lcomplex_t(data.real() * rotCos - data.imag() * rotSin,
                    data.real() * rotSin + data.imag() * rotCos);
}

void CalculateTimeIntegratedGradients(
    lcomplex_t **gainSolutionPerBaseline, long double **visWeights,
    const Data &dataSet, const lcomplex_t *const *gainSolutionPerAntenna) {
  const size_t antennaCount = dataSet.AntennaCount(),
  ...
  for (size_t a1 = 0; a1 != antennaCount; ++a1) {
    for (size_t a2 = a1 + 1; a2 != antennaCount; ++a2) {
      ...
      for (size_t ch = 0; ch != channelCount; ++ch) {
        ...
        lcomplex_t curSolutionY =
            std::conj(*(antenna1Gain + 1)) * *(antenna2Gain + 1);
        lcomplex_t &baselGainX = *baselineGains,
                   &baselGainY = *(baselineGains + 1);
        ...
        for (size_t t = 0; t != timestepCount; ++t) {
          ...
          lcomplex_t gainStepX =
              (correctedX != lcomplex_t(0.0)) ? pX / correctedX : 1.0;
          lcomplex_t gainStepY =
              (correctedY != lcomplex_t(0.0)) ? pY / correctedY : 1.0;
  ...
  for (size_t i = 0; i != count; ++i) {
    *phases = fmodl(*phases - prevPhase, twopi) + prevPhase;
    if (prevPhase - *phases > M_PIl)
      ...
      *phases -= twopi;
    if (std::isfinite(*phases)) prevPhase = *phases;
    phases += step;
  }
}

lcomplex_t AntennaGainStep(size_t antenna, size_t antennaCount, size_t eIndex,
                           const lcomplex_t *const *gainSolutionsPerBaseline,
                           const long double *const *phaseWeights) {
...
long double AntennaWeight(size_t antenna, size_t antennaCount, size_t eIndex,
                          const long double *const *visWeights) {
...
std::pair<long double, long double> ChannelStdError(
    size_t ch, size_t antennaCount, size_t polarizationCount,
    const lcomplex_t *const *gainSolutionsPerBaseline,
    const long double *const *phaseWeights) {
  ...
  for (size_t pol = 0; pol != polarizationCount; ++pol) {
    for (size_t a1 = 0; a1 != antennaCount; ++a1) {
      lcomplex_t gainStep = AntennaGainStep(
          a1, antennaCount, eIndex, gainSolutionsPerBaseline, phaseWeights);

      long double weight =
          AntennaWeight(a1, antennaCount, eIndex, phaseWeights);
  ...
  if (weightSum == 0.0)
    ...
    return std::pair<long double, long double>(sqrtl(stdError / weightSum),
                                               weightSum);
}

void FitPhases(size_t channelCount, size_t polarizationCount,
               long double *phaseOffsets, long double *outputs,
  ...
  for (size_t p = 0; p != polarizationCount; ++p) {
    ...
    size_t x = 0;
    for (size_t eIndex = p; eIndex < eTotal; eIndex += polarizationCount) {
      long double diffx = (long double)x - avgX,
    ...
    long double xSum =
        ((long double)outputChannelCount / channelCount) / 2.0L - 0.5L;
    xSum = xSum * xFactor + alpha;
    for (size_t eIndex = p; eIndex < outputChannelCount * polarizationCount;
         eIndex += polarizationCount) {
...
void SavePhases(const char *outTxtName, const char *outBinName,
                uint64_t antennaCount, uint64_t avgChannelCount,
                uint64_t inpChannelCount, uint64_t polarizationCount,
                const lcomplex_t *const *solutions, bool fitSlope) {
  std::ofstream outFile(outTxtName);
  std::ofstream outBinFile(outBinName, std::ios_base::binary);
  outFile.precision(10);
  outFile << antennaCount << '\t';
  outBinFile.write(reinterpret_cast<char *>(&antennaCount),
                   sizeof(antennaCount));
  if (avgChannelCount == 1) {
    outFile << avgChannelCount << '\t' << polarizationCount << '\n';
    outBinFile.write(reinterpret_cast<char *>(&avgChannelCount),
                     sizeof(avgChannelCount));
    outBinFile.write(reinterpret_cast<char *>(&polarizationCount),
                     sizeof(polarizationCount));
    for (size_t a = 0; a != antennaCount; ++a) {
      ...
      for (size_t p = 0; p != polarizationCount; ++p) {
        if (p == 0 || p == polarizationCount - 1) {
          ...
          double phase =
              atan2l(antennaSols[pIndex].imag(), antennaSols[pIndex].real());
          outFile << '\t' << phase;
    ...
    outBinFile.write(reinterpret_cast<char *>(&outpChannelCount),
                     sizeof(outpChannelCount));
    outBinFile.write(reinterpret_cast<char *>(&polarizationCount),
                     sizeof(polarizationCount));
      ...
      for (size_t p = 0; p != polarizationCount; ++p) {
        if (p == 0 || p == polarizationCount - 1) {
          for (size_t a = 0; a != antennaCount; ++a) {
            const lcomplex_t *antennaSols = solutions[a];
            double phase =
                atan2l(antennaSols[eIndex].imag(), antennaSols[eIndex].real());
            outFile << '\t' << phase;
            outBinFile.write(reinterpret_cast<char *>(&phase), sizeof(phase));
          }
          ++eIndex;
...
void SaveGains(const char *outTxtName, const char *outBinName,
               uint64_t antennaCount, uint64_t avgChannelCount,
               uint64_t inpChannelCount, uint64_t polarizationCount,
               const lcomplex_t *const *solutions, bool fitSlope) {
  /**
      ...
      for (size_t p = 0; p != polarizationCount; ++p) {
        if (p == 0 || p == polarizationCount - 1) {
          for (size_t a = 0; a != antennaCount; ++a) {
            ...
            double gain =
                1.0L / sqrtl(g.real() * g.real() + g.imag() * g.imag());
            outFile << '\t' << gain;
            outBinFile.write(reinterpret_cast<char *>(&gain), sizeof(gain));
          }
          ++eIndex;
...
int main(int argc, char *argv[]) {
  if (argc < 4) {
    std::cout << "Usage: phasecal [-a <nr>] [-f] <model> <measurementset.ms> "
                 "<phases.txt> <gains.txt>\n\n"
              << "This will calculate \"static\" phase offsets for all "
                 "stations. It produces approximate least-squares solutions.\n"
              << "Option -a will average over frequency before fitting, nr "
    ...
    ROArrayColumn<double> phaseDirColumn(
    ...
    size_t reservedSpace =
        channelCount * 2;  // we reserve more for interpolation
    aocommon::UVector<lcomplex_t *> gainStepValues(matrixSize);
    aocommon::UVector<long double *> gainStepWeights(matrixSize);
    ...
    aocommon::UVector<lcomplex_t *> gainSolutionsPerAntenna(antennaCount);
    for (size_t a = 0; a != antennaCount; ++a) {
      gainSolutionsPerAntenna[a] = new lcomplex_t[reservedSpace];
      for (size_t i = 0; i != reservedSpace; ++i)
        gainSolutionsPerAntenna[a][i] = 1.0;
    }
    ...
    const double sampleWeight = 1.0 / channelCount;
    {
      ...
      for (size_t rowIndex = 0; rowIndex != ms.nrow(); ++rowIndex) {
        ...
        // Cross correlation?
        size_t antenna1 = ant1Column.get(rowIndex),
               antenna2 = ant2Column.get(rowIndex);
        if (antenna1 != antenna2) {
          ...
          for (size_t ch = 0; ch != channelCount; ++ch) {
            ...
            for (size_t p = 0; p != polarizationCount; ++p) {
              ...
              if (!(*flagPtr) && std::isfinite(dataPtr->real()) &&
                  std::isfinite(dataPtr->imag())) {
                if (p == 0) {
                  dataSet.Measured(antenna1, antenna2)
                      .DataX(timeIndex, chIndex) += *dataPtr;
                  dataSet.Weights(antenna1, antenna2)
                      .WeightX(timeIndex, chIndex) += sampleWeight;
                } else if (p == polarizationCount - 1) {
                  dataSet.Measured(antenna1, antenna2)
                      .DataY(timeIndex, chIndex) += *dataPtr;
                  dataSet.Weights(antenna1, antenna2)
                      .WeightY(timeIndex, chIndex) += sampleWeight;
                }
              }
              ++dataPtr;
              ++flagPtr;
    ...
    std::cout << "DONE!\nPredicting model with " << model.SourceCount()
              << " sources ... " << std::flush;
    Predicter predicter(phaseCentreRA, phaseCentreDec,
                        bandData.LowestFrequency(), bandData.HighestFrequency(),
                        avgChannelCount);
    ...
    for (size_t rowIndex = 0; rowIndex != ms.nrow(); ++rowIndex) {
      ...
      if (antenna1 != antenna2) {
        ...
        for (size_t ch = 0; ch != avgChannelCount; ++ch) {
          ...
          double lambda =
              bandData.ChannelWavelength(ch * channelCount / avgChannelCount);
          predicter.Predict4(temp, model, u / lambda, v / lambda, w / lambda,
                             ch, antenna1, antenna2);
          ...
          long double avgFactorX =
              weights.WeightX(timeIndex, ch) != 0
                  ? sampleWeight / weights.WeightX(timeIndex, ch)
                  : 0.0;
          long double avgFactorY =
              weights.WeightY(timeIndex, ch) != 0
                  ? sampleWeight / weights.WeightY(timeIndex, ch)
                  : 0.0;
    ...
    CalculateTimeIntegratedGradients(gainStepValues.data(),
                                     gainStepWeights.data(), dataSet,
                                     gainSolutionsPerAntenna.data());

    long double stdError = 0.0, stdErrorWeight = 0.0;
    for (size_t ch = 0; ch != avgChannelCount; ++ch) {
      std::pair<long double, long double> result = ChannelStdError(
          ch, antennaCount, 2, gainStepValues.data(), gainStepWeights.data());
    ...
    do {
      ...
      for (size_t a1 = 0; a1 != antennaCount; ++a1) {
         ...
         */
        size_t eIndex = 0;
        aocommon::UVector<lcomplex_t> gainSteps(sumsPerBaseline);
        aocommon::UVector<long double> antennaWeights(sumsPerBaseline);
        for (size_t ch = 0; ch != avgChannelCount; ++ch) {
          for (size_t p = 0; p != 2; ++p) {
            gainSteps[eIndex] =
                AntennaGainStep(a1, antennaCount, eIndex, gainStepValues.data(),
                                gainStepWeights.data());
            antennaWeights[eIndex] =
                AntennaWeight(a1, antennaCount, eIndex, gainStepWeights.data());

            if (eIndex == 4 && a1 == 5) {
              std::cout << PhaseToString(gainSteps[eIndex]) << '\t'
                        << PhaseToString(gainSolutionsPerAntenna[a1][eIndex])
                        << '\t' << "W=" << antennaWeights[eIndex] << '\t';
            }
            ++eIndex;
          }
        }
        ...
        for (size_t ch = 0; ch != avgChannelCount; ++ch) {
          for (size_t p = 0; p != 2; ++p) {
            gainSolutionsPerAntenna[a1][eIndex] *=
                lcomplex_t(1.0) + (gainSteps[eIndex] - lcomplex_t(1.0)) *
                                      stepSize * antennaWeights[eIndex];
            ++eIndex;
          }
        }
      }
    ...
    // TODO FitPhases(avgChannelCount, 2, antennaPhases, antennaPhases,
    // inpChannelCount);
    // }
    // }

    SavePhases(outNamePhases, "phases.bin", antennaCount, avgChannelCount,
               inpChannelCount, polarizationCount,
               gainSolutionsPerAntenna.data(), fitSlope);
    SaveGains(outNameGains, "gains.bin", antennaCount, avgChannelCount,
              inpChannelCount, polarizationCount,
              gainSolutionsPerAntenna.data(), fitSlope);
