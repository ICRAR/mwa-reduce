...
class LogBinnedPlot {
 public:
  explicit LogBinnedPlot(size_t binsPerOrderOfMagnitude = 4)
      : _binsPerOrderOfMagnitude(binsPerOrderOfMagnitude) {}
  ...
  void Add(const aocommon::UVector<double>& x,
           const aocommon::UVector<double>& y) {
  ...
  void Plot(GNUPlot::Line& line) const {
    for (std::map<int, Bin>::const_iterator i = _bins.begin(); i != _bins.end();
         ++i) {
  ...
  void PlotWithOtherErrorBars(GNUPlot::Line& line,
                              const LogBinnedPlot& plotForErrorBars) const {
  ...
  void GetData(aocommon::UVector<double>& x, aocommon::UVector<double>& y) {
    x.clear();
    y.clear();
 ...
 private:
  struct Bin {
    ...
    double stdErr() const { return sqrt(sumSq) / weight; }
  ...
  int valueCenterToBin(double val) const {
    if (val <= 0.0) return std::numeric_limits<int>::min();
    int bin = 0;
    while (val >= 10.0) {
