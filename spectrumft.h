...
class SpectrumFT {
 public:
  ...
  void GetFTPower(aocommon::UVector<double>& destination,
                  const MeasuredSED& sed, size_t nTermsFitted,
                  bool toTemperature, bool useLomb) const;

  double GetAveragePowerInKRange(double kLow, double kHigh,
                                 const MeasuredSED& sed, size_t nTermsFitted,
  ...
  static void Normalize(aocommon::UVector<double>& ps) {
    double f = 0.0;
    for (aocommon::UVector<double>::iterator i = ps.begin(); i != ps.end(); ++i)
      f += (*i) > 0.0 ? (*i) : 0.0;
    f = double(ps.size()) / f;
    for (aocommon::UVector<double>::iterator i = ps.begin(); i != ps.end(); ++i)
      (*i) *= f;
  }
  static double LowerKParallellEoRWindow() { return 6.5e-2; }
  static double UpperKParallellEoRWindow() { return 3.7; }

  static void ConvertPSToTemperature(aocommon::UVector<double>& ps,
                                     const double* frequencies,
                                     const double* pBeamOmegas) {
    // Good references:
    ...
    std::cout << "distance=" << distance << ", nuToR=" << nuToR
              << ", lToR=" << lToR << '\n';

    double f = frequencies[ps.size() / 2];
    double term = (2.0 * b * f * f) / (c * c);
    double factor = /*flux to watts*/ 1e-23 * /*K^2 to mK^2*/ 1e-6 *
                    (lToR * lToR * nuToR) /
                    (term * term * bandwidth * centralOmega);

    for (size_t channelIndex = 0; channelIndex != ps.size(); ++channelIndex) {
      if (channelIndex == ps.size() / 2)
        std::cout << "Channel " << channelIndex
                  << " ps to temp factor = " << factor << '\n';
      ps[channelIndex] *= factor;
    }
  }

  void ConvertPSToTemperature(aocommon::UVector<double>& ps) {
    ConvertPSToTemperature(ps, _frequencies.data(), _pBeamOmegas.data());
  }

  static void ConvertFluxToTemperature(aocommon::UVector<double>& data,
                                       const double* frequencies,
                                       const double* sBeamOmegas) {
    const double c = Universe::C(), b = Universe::Boltzmann();
    for (size_t channelIndex = 0; channelIndex != data.size(); ++channelIndex) {
      double f = frequencies[channelIndex];
      // double lambda = c/f;
      // conversion to mK
      double jToT = /*2.0*M_PI **/ 1e-23 * c * c /
                    (2.0 * b * f * f * sBeamOmegas[channelIndex]);
      data[channelIndex] *= jToT;
    }
  }

  void ConvertFluxToTemperature(aocommon::UVector<double>& data) {
    ConvertFluxToTemperature(data, _frequencies.data(), _sBeamOmegas.data());
  }

  void SetMetaData(const MeasuredSED& sed, double maximumBaselineInM,
                   size_t sourceCount) {
    aocommon::UVector<double> frequencies(sed.MeasurementCount());
  ...
  void SetMetaData(const aocommon::UVector<double>& frequencies,
                   double maximumBaselineInM, size_t sourceCount) {
    ...
    for (size_t i = 0; i != frequencies.size(); ++i) {
      ...
      if (i == frequencies.size() / 2) {
        std::cout << "Synthesized beam at " << f * 1e-6
                  << " MHz: " << aocommon::units::Angle::ToNiceString(sBeam)
                  << " FWHM (omega=" << _sBeamOmegas[i] << ")\n";
        double pb = sqrt(_pBeamOmegas[i] / (M_PI / (4.0 * M_LN2)));
        std::cout << "Effective primary beam at " << f * 1e-6
                  << " MHz: " << aocommon::units::Angle::ToNiceString(pb)
                  << " FWHM (omega=" << _pBeamOmegas[i] << ")\n";
      }
    }
  }
  void SetOutputFit(bool outputFit) { _outputFit = outputFit; }
  void PreparePSPlot(class GNUPlot& plot, double centralFrequency,
                     bool useDelay);

 private:
  void transformSpectrum(aocommon::UVector<std::complex<double>>& data,
                         size_t sampleCount) const;
  void lombPeriodogram(aocommon::UVector<double>& output,
                       const aocommon::UVector<double>& input) const;
  void twoPoint(aocommon::UVector<double>& output,
                const aocommon::UVector<double>& input) const;
  void applyWindow(aocommon::UVector<double>& data) const;
  static double getKForFreqStep(double freqStepHz, double centralFreq);
  void interpolate(aocommon::UVector<double>& data, size_t i) const;
  void interpolate(aocommon::UVector<double>& data) const;

  size_t _channelCount;
  fftw_plan _fftPlan;
  bool _outputFit;
  aocommon::UVector<double> _frequencies;
  aocommon::UVector<double> _sBeamOmegas, _pBeamOmegas;
